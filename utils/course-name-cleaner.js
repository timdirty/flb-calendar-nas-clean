...
function cleanCourseName(courseName) {
    ...
    // 例如：［代課］［改時間] SPIKE 五 1610-1740 -> SPIKE 五 1610-1740
    // 使用迴圈確保移除所有標記
    ...
    // 🔥 [修復 2025-11-27] 修復時間格式統一後的空格問題
    // 確保時間和星期之間有空格：BOOST 四1530-1700 -> BOOST 四 1530-1700
    cleaned = cleaned.replace(/([一二三四五六日])\s*(\d{3,4}-\d{3,4})/g, '$1 $2');
    
    // 移除各種格式的週次標記
    // 例如：第8週、第10週、week 8、Week 10 等
    cleaned = cleaned
        .replace(/\s+第\d+週/gi, '')      // 中文週次（第X週，有空白前綴）
        .replace(/第\d+週/gi, '')          // 中文週次（第X週，無空白前綴）
        .replace(/\s+第.{1,3}週/gi, '')   // 中文週次（第一週、第二週等）
        .replace(/第.{1,3}週/gi, '')       // 中文週次（第一週、第二週等，無空白前綴）
        .replace(/\s+week\s*\d+/gi, '')   // 英文週次（week X）
        .replace(/week\s*\d+/gi, '')       // 英文週次（week X，無空白前綴）
        .replace(/\s+w\d+/gi, '')         // 簡寫週次（w8）
        .replace(/w\d+/gi, '')             // 簡寫週次（w8，無空白前綴）
        .replace(/\s+週\d+/gi, '')        // 週X格式
        .replace(/週\d+/gi, '')            // 週X格式（無空白前綴）
