...
// 引入依賴（Node.js 環境）
let _metadataDateFormatter, _metadataSemesterHelper, _metadataDrivePathHelper;
if (typeof require !== 'undefined') {
    _metadataDateFormatter = require('./date-formatter');
    _metadataSemesterHelper = require('./semester-helper');
    _metadataDrivePathHelper = require('./drive-path-helper');
}
...
class MetadataTransformer {
     ...
     */
    constructor() {
        this.dateFormatter = _metadataDateFormatter || (typeof window !== 'undefined' ? window.DateFormatter : null);
        this.semesterHelper = _metadataSemesterHelper || (typeof window !== 'undefined' ? window.SemesterHelper : null);
        this.drivePathHelper = _metadataDrivePathHelper || (typeof window !== 'undefined' ? window.DrivePathHelper : null);
    }
     ...
     * 提取學期
     * 規則：
     * 1. 若呼叫端已提供 semester（無論格式是「2025上學期」或「114-1」），一律信任並直接回傳。
     * 2. 僅在完全沒有提供 semester 時，才依日期透過 SemesterHelper 推算預設學期。
     */
    extractSemester(input) {
        ...
        // 若已有明確的學期字串，直接使用（避免將「2025上學期」轉成「114-1」）
        if (semester) {
            return String(semester).trim();
        }

        // 從日期推算學期（僅在未提供 semester 時）
        const date = this.extractDate(input);
