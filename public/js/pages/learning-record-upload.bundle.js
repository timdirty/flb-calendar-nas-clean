...
(function (global) {
  ...
  function buildDriveProxyPath(pathValue, filename) {
    try {
      ...
      var segments = [];
      function pushSegments(source) {
        if (!source) return;
        var clean = String(source || '').replace(/\\/g, '/').replace(/\/{2,}/g, '/');
        clean.split('/').forEach(function (seg) {
          if (!seg) return;
          var normalized = seg;
          try { normalized = decodeURIComponent(seg); } catch (e) {}
          var sanitized = sanitizeDriveSegment(normalized);
          if (sanitized) {
            segments.push(sanitized);
          }
        });
      }
      pushSegments(absolutePath);
      if (filename) {
        pushSegments(filename);
      }
      if (!segments.length) return '';
  ...
  function buildDirectFileUrl(relativePath, filename, options) {
    ...
    var normalizedRelativePath = normalizeDriveRelativePath(relativePath, contextRecord, cacheEntry);
    if (!normalizedRelativePath) {
      console.warn('⚠️ buildDirectFileUrl: 無法標準化路徑', { filename: actualFilename, relativePath, record: contextRecord && contextRecord.studentName });
      return '';
    }
    
    var absolutePath = ensureDriveAbsolutePath(normalizedRelativePath);
    var proxyPath = buildDriveProxyPath(absolutePath, actualFilename);
    if (!proxyPath) {
      console.warn('⚠️ buildDirectFileUrl: 無法構建代理路徑', { relativePath: normalizedRelativePath, actualFilename });
      return '';
    }
  ...
  async function fetchStudentFsRecord(index, student) {
    try {
      ...
      if (historyResult && historyResult.records && historyResult.records.length > 0) {
        ...
        if (!recordRaw) {
          recordRaw = historyResult.records[0];
        }
        r = {
          success: true,
          found: true,
      ...
      if (r && r.success && r.found) {
        ...
        if (i >= 0) list[i] = studentRecord; else list.push(studentRecord);
        cache.students = list;
  ...
  function applyExistingRecordToCard(index, student, uploaded) {
    ...
    try {
      ...
      // 🧹 清除舊草稿，避免重新載入時又套用已經上傳過的評語導致重複上傳
      try {
        if (window.FLB && FLB.State) {
          var stDraft = FLB.State.get();
          if (stDraft && stDraft.drafts && stDraft.drafts.hasOwnProperty(String(index))) {
            var draftsClean = Object.assign({}, stDraft.drafts);
            delete draftsClean[String(index)];
            FLB.State.set({ drafts: draftsClean });
            console.log('🧹 [applyExistingRecordToCard] 已清除學生草稿，避免重複自動上傳:', student.name);
          }
        }
      } catch (draftErr) {
        console.warn('⚠️ [applyExistingRecordToCard] 清除草稿失敗:', draftErr);
      }
      ...
      if (videosPreview) {
        ...
        validVideos.forEach(function (item) {
          ...
          // 🔥 也處理後端返回的物件格式（例如 {name: 'xxx.mp4', path: '...', size: ...}）
          var isNewSystem = (typeof item === 'object' && item !== null && item.id);
          var filename;
          if (typeof item === 'string') {
            filename = item;
    ...
    try {
      ...
      if (window.SharedIntegration && typeof window.SharedIntegration.handleStudentMediaSelect === 'function') {
        try {
          ...
          // 🔥 [修復] 使用共用預覽器渲染，不要調用 updateFilePreviews
          if (window.SharedMediaPreviewer && entry[type].length > 0) {
            try {
              var container = document.getElementById(type + '-preview-' + studentIndex);
              if (container) {
                console.log('🎨 [handleFileSelect] 準備渲染預覽:', {
                  studentIndex: studentIndex,
                  type: type,
                  filesCount: entry[type].length,
                ...
                console.log('✅ [handleFileSelect] 預覽渲染完成:', {
                  containerChildren: container.children.length,
                  newUploadNodes: container.querySelectorAll('.file-preview.new-upload').length
                });
              }
            } catch (previewErr) {
              console.warn('⚠️ 渲染預覽失敗:', previewErr);
            }
          }
      ...
      files.forEach(function(file) {
        ...
        if (type === 'photos') {
          var isImage = allowedImageTypes.indexOf(fileType) >= 0 || 
          ...
          var isVideo = allowedVideoTypes.indexOf(fileType) >= 0 || 
      ...
      for (var i = 0; i < files.length; i++) {
        try {
          ...
          console.error('❌ 處理檔案失敗:', file.name, fileErr);
          // 失敗也要加入（使用原檔案），不中斷流程
  ...
  async function uploadStudentRecord(studentIndex) {
    ...
    // 🔥 異步查詢 DOM 元素（避免阻塞主線程）
    var btn = await new Promise(resolve => {
      updateDOMAsync(() => {
        var btn = document.getElementById('upload-btn-' + studentIndex);
    ...
    try {
      ...
      // 📋 获取学期信息（从系统设置或默认值）
      var currentSemester = '114-1'; // TODO: 从系统设置获取
      try {
        if (window.systemSettings && window.systemSettings.currentSemester) {
          currentSemester = window.systemSettings.currentSemester;
        }
      } catch (e) {}
      ...
      // 🔥 統一使用 currentCourse.title 作為 courseName（與課程總覽一致）
      // 這樣可以確保學生頁面和課程總覽使用相同的資料夾命名
      var finalCourseName = String(currentCourse.title || '').trim();
      
      // 構建元數據（確保所有必要欄位都有值）
      var metadata = {
        semester: String(currentSemester).trim(),
        courseName: finalCourseName, // 🔥 使用完整的日曆標題，與課程總覽一致
        date: String(dateValue).trim(),
        studentName: String(student.name || '').trim()
      };
      
      // 🔥 驗證元數據完整性
      if (!metadata.semester || !metadata.courseName || !metadata.date || !metadata.studentName) {
        console.error('❌ [Drive 上傳] 元數據不完整:', metadata);
        throw new Error('元數據不完整: ' + JSON.stringify(metadata));
      }
      
      console.log('📋 [Drive 上傳] 元數據:', metadata);
      
      if (pendingComment && includeComment) {
        metadata.comment = String(pendingComment).trim();
      }
      
      // 🔥 使用共用上傳模組
      // 注意：學生有兩個容器（照片和影片分開），我們需要分別處理
      var photosContainer = '#photos-preview-' + studentIndex;
      var videosContainer = '#videos-preview-' + studentIndex;
      
      // 為照片和影片分別設置進度條
      if (includePhotos && pendingPhotos.length > 0) {
        setupFileProgressBars(pendingPhotos, [], photosContainer);
      }
      if (includeVideos && pendingVideos.length > 0) {
        setupFileProgressBars([], pendingVideos, videosContainer);
      }
      
      try {
        var uploadResult = await uploadFilesWithSmartManager({
          photos: includePhotos ? pendingPhotos : [],
          videos: includeVideos ? pendingVideos : [],
          metadata: metadata,
          containerSelector: null, // 學生有兩個容器，手動處理進度條
          onProgress: function(percent, completed, total) {
            updateDOMAsync(function() {
              var percentRounded = Math.round(percent);
              updateUploadProgressDisplay(studentIndex, percentRounded);
              var bar = document.getElementById('cap-bar-' + studentIndex);
              var fill = bar && bar.querySelector('.fill');
              if (fill) fill.style.width = percentRounded + '%';
              console.log('📤 [學生上傳] 進度:', percentRounded + '%');
              
              // 更新照片和影片的進度條
              if (includePhotos && pendingPhotos.length > 0) {
                updateAllFileProgressBars(photosContainer, percentRounded);
              }
              if (includeVideos && pendingVideos.length > 0) {
                updateAllFileProgressBars(videosContainer, percentRounded);
              }
            });
          },
          onSuccess: function(result) {
            console.log('✅ [Drive 上传] 上传成功:', result);
            ...
            if (includeComment) baseState.existingCounts.text = pendingComment.length;
            
            // 更新所有進度條為 100%（updateAllFileProgressBars 內部已使用 updateDOMAsync）
            if (includePhotos && pendingPhotos.length > 0) {
              updateAllFileProgressBars(photosContainer, 100);
            }
            if (includeVideos && pendingVideos.length > 0) {
              updateAllFileProgressBars(videosContainer, 100);
            }
            
            // 短暫延遲後移除預覽（讓用戶看到完成狀態）
            setTimeout(function() {
              if (includePhotos && pendingPhotos.length > 0) {
                cleanupUploadUI(pendingPhotos, [], photosContainer);
              }
              if (includeVideos && pendingVideos.length > 0) {
                cleanupUploadUI([], pendingVideos, videosContainer);
              }
            }, 500);
            ...
            // 🔄 僅刷新當前學生的檔案狀態，避免整頁重載
            setTimeout(function() {
              refreshStudentRecordByIndex(studentIndex, { fallbackToFullReload: true });
            }, 600);
          ...
          onError: function(error) {
            console.error('❌ [Drive 上传] 失败:', error);
            // 🔥 異步恢復 UI（避免阻塞主線程）
            updateDOMAsync(function() {
              btn.classList.remove('uploading');
              btn.classList.add('error');
              btn.disabled = false;
              btn.innerHTML = '<i class="fas fa-exclamation-triangle"></i> 上传失败，点此重试';
              updateUploadProgressDisplay(studentIndex, 0);
            }, 0);
            if (typeof showToast === 'function') {
        ...
        // 捕获整个上传流程中的错误
        console.error('❌ [Drive 上传] 发生异常:', error);
        // 🔥 異步恢復 UI（避免阻塞主線程）
        updateDOMAsync(function() {
          btn.classList.remove('uploading');
          btn.classList.add('error');
          btn.disabled = false;
          btn.innerHTML = '<i class="fas fa-exclamation-triangle"></i> 上传异常，点此重试';
        }, 0);
        uploadingStudents[studentIndex] = false;
        if (typeof showToast === 'function') {
    ...
    return; // ✅ Drive 上传路径完成，不再执行下面的旧代码
  ...
  async function refreshStudentRecordByIndex(studentIndex, opts) {
    ...
    try {
      ...
      if (opts && opts.fallbackToFullReload) {
...
function updateIndividualOverviewFileProgress(photos, videos, loaded, total, percentFallback) {
    try {
      ...
      allFiles.forEach(function(fileInfo) {
        ...
        // 🔥 更新 UI - 增加容错：如果找不到元素，稍后重试
        var preview = document.querySelector(fileInfo.selector);
        
        if (!preview) {
          console.warn('⚠️ [DOM 查詢] 找不到預覽元素，嘗試其他選擇器:', fileInfo.selector);
          ...
          if (container) {
            ...
            for (var i = 0; i < allPreviews.length; i++) {
              if (parseInt(allPreviews[i].getAttribute('data-file-index') || '-1', 10) === fileInfo.index) {
                ...
                console.log('✅ [容錯] 通過 data-file-index 找到元素');
                break;
              }
            }
          }
          ...
          if (!preview && container) {
            var allPreviews = container.querySelectorAll('.file-preview');
            if (allPreviews[fileInfo.index]) {
              preview = allPreviews[fileInfo.index];
              console.log('✅ [容錯] 通過位置索引找到元素');
            }
          }
        }
        ...
        if (preview) {
          ...
          if (progressFill) {
            ...
            console.log('✅ [進度條] 已更新寬度:', fileProgress + '%');
          } else {
            console.warn('⚠️ [進度條] 找不到 progressFill');
          }
          
          if (progressText) {
            if (fileProgress >= 100) progressText.textContent = '✓ 完成';
            else if (fileProgress === 0) progressText.textContent = '排隊中…';
            else progressText.textContent = fileProgress + '%';
          }
          
          // 🔥 上傳完成後隱藏旋轉動畫
          if (fileProgress >= 100 && thumbLoading) {
      ...
      if (e && e.stack) console.error('錯誤堆疊:', e.stack);
    }
  }
    ...
    updateDOMAsync(() => {
      try {
        ...
        function markPreviewSynced(preview) {
          ...
          var progressText = preview.querySelector('.progress-text');
          if (progressText) progressText.textContent = '已同步';
...
function cleanupOverviewUploadUI(photos, videos) {
    cleanupUploadUI(photos, videos, '#overviewPhotosPreviews', { keepPreview: true });
  }
  
  // 🔥 2025-11-08: 重構為智能上傳管理器
  async function uploadOverview(options) {
    ...
    try {
      ...
      var uploadResult = await uploadFilesWithSmartManager({
        ...
        videos: videos,
        metadata: metadata,
        containerSelector: '#overviewPhotosPreviews',
        onProgress: function(percent, completed, total) {
          updateDOMAsync(function() {
            updateOverviewUploadUI(percent, completed, total, silent, btn, syncFill, syncText);
          });
        },
        onMemoryWarning: function(level, available) {
          console.warn('⚠️ [內存警告] 等級:', level, '可用:', available.toFixed(0) + 'MB');
          
          if (level === 'critical') {
            showToast('⚠️ 內存不足，上傳速度已降低', 'warning');
          } else if (level === 'high') {
            showToast('⚠️ 內存緊張，已切換為單文件上傳', 'warning');
          }
        },
        onSuccess: function(result) {
          console.log('✅ [智能上傳] 上傳成功:', result);
          
          // 短暫延遲後移除預覽（讓用戶看到完成狀態）
          setTimeout(function() {
            cleanupOverviewUploadUI(photos, videos);
          }, 500);
          
          // 清空待上傳列表
          window.overviewPhotosFiles = [];
          window.overviewVideosFiles = [];
          ...
          updateDOMAsync(function() {
            ...
            if (!silent && btn) {
              btn.classList.remove('uploading');
              btn.classList.add('success');
              btn.innerHTML = '<i class="fas fa-check"></i> 上傳成功 (' + result.completed + '/' + result.total + ')';
            }
            if (syncText) {
              syncText.textContent = '✅ 同步完成 (' + result.completed + '/' + result.total + ')';
            ...
            // 清空新上傳預覽容器
            try {
              var photoPreview = document.getElementById('overviewPhotosPreviews');
              if (photoPreview) {
                photoPreview.innerHTML = '';
                console.log('✅ [UI] 已清空新上傳預覽容器');
              }
            } catch (e) {
              console.error('❌ [UI] 清空預覽容器失敗:', e);
            }
          }, 0);
          ...
          if (typeof showToast === 'function') {
            if (result.failed > 0) {
              showToast('⚠️ 部分上傳失敗：成功 ' + result.completed + '，失敗 ' + result.failed, 'warning');
            } else {
              showToast('✅ 上傳成功！共 ' + result.completed + ' 個文件', 'success');
            }
          }
          
          // 🔥 異步重新載入課程總覽數據（與學生頁面一致）
          setTimeout(function() {
            updateDOMAsync(function() {
              try {
                // 1. 清除快取，強制重新載入
                if (typeof uploadedCacheHydratedAt !== 'undefined') {
                  uploadedCacheHydratedAt = 0;
                  console.log('🗑️ [快取] 已清除課程總覽快取');
                }
                
                // 2. 重新載入記錄
                if (typeof loadUploadedRecordsForCurrentCourse === 'function') {
                  loadUploadedRecordsForCurrentCourse({ force: true }).then(() => {
                    // 3. 載入完成後才觸發 render（避免重複調用）
                    if (currentCourse && typeof window.LearningOverviewRenderer !== 'undefined') {
                      window.LearningOverviewRenderer.render(currentCourse, { skipExisting: true, force: true });
  ...
  async function uploadOverviewTextOnly(options) {
    ...
    try {
      ...
      // 添加總覽摘要作為 comment
      if (autoBlock) {
        formData.append('comment', autoBlock);
      }
      
      var response = await fetch('/api/learning-records/upload-drive', {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
  ...
  async function loadUploadedRecordsForCurrentCourse(opts) {
    ...
    var shouldShowLoader = !hasRenderedUploadedRecords || (opts && opts.showLoader);
    ...
    try {
      ...
      */
      ...
      const isNotFound = e && (
        e.message && (e.message.includes('408') || e.message.includes('404') || e.message.includes('不存在'))
      );
      
      if (isNotFound) {
        console.log('ℹ️ [載入記錄] Drive 目錄不存在');
        if (!hasRenderedUploadedRecords) {
          ...
          showToast('找不到最新 Drive 資料，已保留上一版內容', 'warning');
        }
      } else {
        console.warn('⚠️ [載入記錄] 載入失敗:', e.message || e);
        showToast('載入學習記錄失敗：' + (e.message || '請稍後再試'), 'error');
      }
      if (shouldShowLoader) {
      ...
      if (shouldShowLoader && !loaderDismissed) {
        setRecordsLoadingState(false);
      }
  ...
  function renderUploadedRecords(data, options) {
    ...
    if (dataset.overview) {
      ...
      function buildUrl(filename){
        ...
        return buildDirectFileUrl(rel, filename, { record: dataset.overview });
      }
      ...
      var videoBlocks = videoItems.map(function(vItem){
        ...
        if (isNewSystem) {
          ...
          var legacyThumb = overviewThumbMap && overviewThumbMap[filename];
          thumbUrl = legacyThumb ? buildUrl(legacyThumb) : '';
        }
      ...
      overviewHtml = '<div class="record-overview"><div>' + metaLine + '</div>' +
        (dataset.overview.summary ? ('<div class="record-overview-summary">' + global.FLB.Course.escapeHtml(dataset.overview.summary.substring(0, 120)) + (dataset.overview.summary.length > 120 ? '...' : '') + '</div>') : '') +
        grid +
        '</div>';
      try {
      ...
      try {
        if (!overviewFromCache && typeof hydrateOverviewFieldsFromSummary === 'function' && dataset.overview && typeof dataset.overview.summary === 'string' && dataset.overview.summary.trim()) {
          console.log('🔄 [回填表單] 開始回填課程總覽數據...');
          hydrateOverviewFieldsFromSummary(dataset.overview.summary, { force: true });
          lastTextSnapshot = computeOverviewTextSnapshot();
          lastOverviewSnapshot = computeOverviewSnapshot();
        }
      } catch (e) {
  ...
  async function deleteOverviewFile(triggerEl, filename) {
    ...
    try {
      ...
      try {
        ...
        console.log('✅ [deleteOverviewFile] 已重新載入資料');
      } catch (e) {
        console.error('❌ [deleteOverviewFile] 重新載入失败:', e);
      }
      return true;
    } catch (e) {
      console.error(e);
