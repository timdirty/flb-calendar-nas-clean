(function (global) {
  ...
  (function setupGlobalErrorHandlers() {
    ...
    window.addEventListener('error', function(event) {
      ...
      console.error('❌ [錯誤堆疊]', event.error && event.error.stack);
  ...
  (function ensureOverviewNoSpinnerStyles(){
    try {
      if (document.getElementById('overview-no-spinner-style')) return;
      var style = document.createElement('style');
      style.id = 'overview-no-spinner-style';
      style.textContent = [
        '#overviewPhotosPreviews .thumb-loading,',
        '#overviewExistingPreviews .thumb-loading{display:none!important}',
        '#overviewPhotosPreviews .file-preview.loading .file-uploading-overlay,',
        '#overviewExistingPreviews .file-preview.loading .file-uploading-overlay{opacity:1!important;pointer-events:auto!important}',
        '#overviewPhotosPreviews .file-preview.loading img,',
        '#overviewPhotosPreviews .file-preview.loading video,',
        '#overviewExistingPreviews .file-preview.loading img,',
        '#overviewExistingPreviews .file-preview.loading video{opacity:1!important}'
      ].join('');
      var inject = function(){ try { document.head.appendChild(style); } catch(e){} };
  ...
  var USE_GLOBAL_PROGRESS_TOAST = false;
  var ENABLE_FLOATING_PROGRESS_INDICATOR = false;
  var AUTO_REFRESH_AFTER_UPLOAD = false;
  ...
  var processingVideosCheckTimer = null; // ⏰ 處理中影片檢查計時器
        ...
        if (typeIsVideo && placeholder) {
          ...
          if (localSrc && localSrc.indexOf('blob:') === 0) {
            ...
            if (video) {
              setTimeout(function() {
                try {
                  SmartPosterGenerator.processImmediate(video);
                } catch (e) {
                  console.warn('⚠️ [學生卡片] 縮圖生成失敗:', e);
                }
              }, 150);
            }
  ...
  function silentCheckStudentPhotoThumbnails(studentIndex, studentName) {
    ...
    return fetch('/api/learning-records/lookup-student?' + params.toString())
      ...
      .then(function(data) {
        ...
        var photos = record.files && record.files.photos ? record.files.photos : [];
        
        // 🔍 檢查是否有照片檔案
        console.log('🔍 [照片-靜默輪詢] 學生:', studentName, '照片數量:', photos.length);
        
        if (!photos.length) return false;  // 沒有照片，不更新
        
        // 🔥 只更新照片方塊的 src（不重繪整個卡片）
        ...
        photoPreviews.forEach(function(preview) {
          ...
          // 如果已經是後端 URL，跳過
          if (currentSrc.indexOf('/api/media/photos/') > -1 || 
          ...
          if (currentSrc.indexOf('blob:') === 0) {
            ...
            if (!filename && record.files && record.files.photos.length > updatedCount) {
              filename = record.files.photos[updatedCount];
            }
            
            if (filename) {
              // 構建後端縮圖 URL
              var thumbnailUrl = buildDirectFileUrl(record.relativePath, filename);
    ...
    return fetch('/api/learning-records/lookup-student?' + params.toString())
      ...
      .then(function(data) {
        ...
        videoPreviews.forEach(function(preview) {
          ...
          // 從 videoUrl 提取 mediaId 或影片檔名
          var videoKey = null;
          if (videoUrl.indexOf('/api/media/videos/') > -1) {
            // 新媒體系統：提取 mediaId
            var parts = videoUrl.split('/api/media/videos/')[1].split('/');
            videoKey = parts[0];  // mediaId
            
            // 查找新媒體系統的縮圖（通過 mediaId 獲取縮圖 URL）
            var thumbUrl = '/api/media/videos/' + videoKey + '/thumbnail';
            ...
            if (posterImg) {
              // 🔥 檢查是否已經載入成功（避免重複更新導致閃爍）
              if (posterImg.src && posterImg.style.display !== 'none' && posterImg.complete) {
                console.log('⏭️ [影片-靜默更新] 縮圖已存在，跳過:', videoKey);
                return;  // 已有縮圖，跳過
              }
              
              // 只更新 src，不重繪整個卡片
              posterImg.src = thumbUrl + '?t=' + Date.now();  // 加時間戳避免緩存
              ...
              // 🔥 更新緩存
              var thumbCacheKey = studentName + ':' + videoKey;
              videoThumbnailReadyCache[thumbCacheKey] = true;
              
              updatedCount++;
              console.log('✅ [影片-靜默更新] 縮圖:', videoKey);
            }
          }
        });
  ...
  function buildMediaPreviewHtml(options) {
    ...
    if (type === 'image') {
      ...
      if (thumbUrl) {
        ...
        innerHtml += '<img class="video-poster" alt="poster" style="display:none;" />';
    ...
    return '<div class="' + classes.join(' ') + '"' + attrs + '>' + innerHtml + loadingHtml + removeHtml + mediaTypeLabel + '</div>';
  }
  ...
  function parseUrlParams() {
    try {
      ...
      return {
        ...
        course: (sp.get('course') || '').trim(),        // 🔥 新增：課程名稱
        topic: (sp.get('topic') || '').trim(),          // 🔥 新增：課程主題
        student: (sp.get('student') || '').trim(),      // 🔥 新增：學生名稱
        autoLoad: (sp.get('autoLoad') || '').trim()     // 🔥 新增：自動載入標記
      };
    } catch (e) {
  ...
  function resolveStudentNameByIndex(idx) {
    try {
      if (typeof idx !== 'number') return '';
      var state = (window.FLB && FLB.State && typeof FLB.State.get === 'function') ? FLB.State.get() : null;
  ...
  var UploadEventLogger = (function () {
    var buffer = [];
    function log(eventName, payload) {
      try {
        ...
        if (typeof data.studentIndex === 'number' && !data.studentName) {
          data.studentName = resolveStudentNameByIndex(data.studentIndex);
        }
        buffer.push(data);
        if (buffer.length > 500) buffer.shift();
        window.__uploadEventBuffer = buffer;
        if (console && typeof console.debug === 'function') {
          console.debug('UPLOAD_EVENT', data.event, data);
        } else {
          console.log('UPLOAD_EVENT', data.event, data);
        }
      } catch (err) {}
    }
    return {
    ...
    update: function (studentIndex, type, fileIndex, percent) {
      try {
        ...
        if (t) {
          ...
          if (!t.__lastLoggedProgress || Math.abs(t.__lastLoggedProgress - t.percent) >= 10 || t.percent === 100) {
            UploadEventLogger.log('progress', {
              studentIndex: studentIndex,
              type: type,
              fileIndex: fileIndex,
              progress: t.percent
            });
            t.__lastLoggedProgress = t.percent;
          }
          this.render();
        }
      } catch (e) {}
      ...
      try {
        ...
        if (idx > -1) {
          // 🔥 [修復] 直接移除任務，不再延遲 800ms
          this.tasks.splice(idx, 1);
          UploadEventLogger.log('done', {
            studentIndex: studentIndex,
            type: type,
            fileIndex: fileIndex
          });
          this.render();
          console.log('✅ [UploadCenter.done] 已移除任務:', sid);
        }
      } catch (e) {}
    },
      ...
      try {
        ...
        if (fab) {
          if (actives.length) {
            ...
            fab.style.display = 'block'; // 顯示 FAB
        ...
        if (!this.tasks.length) {
          ...
          // 自動關閉抽屜
          console.log('✅ [UploadCenter] 所有任務已完成，自動關閉抽屜');
          this.close();
          return;
        }
        // 排序：上傳中 → 失敗 → 完成（新者在上）
  ...
  function composeRelativePath(info, studentName, cacheMeta) {
    ...
    var topic = '';
    if (currentCourse) {
      topic = extractCourseTopicForPath(currentCourse);
    }
    
    // 🔥 組合日期與主題（格式：「YYYY-MM-DD 主題」或只有日期）
    var dateFolder = date;
    if (date && topic) {
      dateFolder = date + ' ' + topic;
    }
    
    // 🔍 除錯：輸出路徑組合資訊（特別是課程總覽）
    if (normalizedStudent === '課程總覽') {
  ...
  function buildRecordFileUrl(record, filename, options) {
    ...
    if (typeof filename === 'object' && filename !== null) {
      ...
      // 嘗試常見的屬性名稱
      var actualFilename = filename.filename || filename.name || filename.originalname || filename.originalName;
    ...
    if (record.newMediaVideos && Array.isArray(record.newMediaVideos)) {
      ...
      if (video && video.id) {
        if (isThumbnail) {
          console.log('🎬 [新媒體系統] 使用影片縮圖 API:', video.id, filename);
          return '/api/media/videos/' + video.id + '/thumbnail';
        }
        console.log('🎬 [新媒體系統] 使用影片下載 API:', video.id, filename);
    ...
    if (record.newMediaPhotos && Array.isArray(record.newMediaPhotos)) {
      ...
      if (photo && photo.id) {
        ...
        var url = '/api/media/photos/' + photo.id + '/preview?date=' + encodeURIComponent(date) + 
    ...
    if (record.mediaId || record.videoMediaId) {
      ...
      if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(mediaId))) {
        console.log('🎬 [舊格式] 使用新媒體影片 API:', mediaId, filename);
        return '/api/media/videos/' + mediaId + '/download';
      }
    }
    
    // ⚠️ 降級到舊系統（僅用於兼容歷史數據）
    console.warn('⚠️ [buildRecordFileUrl] 降級使用舊 API 路徑:', filename);
  ...
  function buildRecordOperationMeta(studentName) {
    ...
    if (!relativePathValue) {
      // 🔥 確保傳入完整的 info 物件（包含 coursePeriod、date、semester）
      var fullInfo = {
        coursePeriod: coursePeriodValue,
        date: dateValue,
        semester: (info && info.semester) || (cacheMeta && cacheMeta.semester) || getCurrentSemester()
      };
      relativePathValue = composeRelativePath(fullInfo, studentName, cacheMeta);
      ...
      if (studentName === '課程總覽') {
        console.log('📁 [buildRecordOperationMeta] 課程總覽路徑生成:', {
          ...
          topic: currentCourse ? extractCourseTopicForPath(currentCourse) : 'no-course'
        });
      }
    }
  ...
  function autoSelectTargetCourse(courses, fallbackTime) {
    ...
    if (!eventId && !(date && time) && !(course && date && autoLoad)) {
      console.warn('⚠️ [autoSelectTargetCourse] 缺少必要參數（eventId 或 date+time 或 course+date+autoLoad）');
      return;
    }
    ...
    if (course && date && autoLoad) {
      ...
      var targetCourseType = extractCourseType(normalizedCourse);
  ...
  async function selectCourse(course, element) {
    ...
    try {
      ...
      // 🔥 設置強制上傳標記，確保清空狀態能被保存（繞過防重複檢查）
      forceNextOverviewUpload = true;
      
      // 🔥 延遲觸發一次自動儲存，確保清空的狀態被保存
      setTimeout(function() {
        try {
          if (typeof uploadOverview === 'function') {
            uploadOverview({ silent: true, force: true });
            console.log('✅ 已觸發課程總覽自動儲存（清空狀態）');
          }
        } catch (e) {
          console.error('❌ 觸發自動儲存失敗:', e);
        }
      }, 500);
    ...
    try {
      ...
      if (r && r.success && r.found) {
        ...
        var record = { 
          ...
          semester: r.semester,
          videoThumbnails: r.videoThumbnails || (r.files && r.files.videoThumbnails) || {},  // 🔥 保留影片缩图映射
  ...
  function setupStudentCard(index, student) {
    ...
    try {
      if (window.LearningUploadStudentStore && typeof window.LearningUploadStudentStore.subscribe === 'function') {
        if (studentStoreSubscriptions[index]) {
          studentStoreSubscriptions[index]();
        }
        studentStoreSubscriptions[index] = window.LearningUploadStudentStore.subscribe(index, function () {
        ...
        notifyStudentStateChange(index);
      }
    } catch (e) {}
    ...
    if (commentArea) {
      ...
      commentArea.addEventListener('blur', function (e) {
        ...
        if (hasPendingChanges(index)) {
          commentArea.classList.add('comment-saved-flash');
          setTimeout(function() {
            commentArea.classList.remove('comment-saved-flash');
          }, 2000);
        }
  ...
  function applyExistingRecordToCard(index, student, uploaded) {
    console.log('🔄 [applyExistingRecordToCard] 開始:', { index: index, student: student.name, hasPhotos: uploaded.files && uploaded.files.photos ? uploaded.files.photos.length : 0 });
    try {
      ...
      if (uploaded.newMediaVideos && Array.isArray(uploaded.newMediaVideos) && uploaded.newMediaVideos.length > 0) {
        ...
        // 🔥 沒有新媒體影片時，才使用舊系統資料
        existingVideos = (uploaded.files && Array.isArray(uploaded.files.videos)) ? uploaded.files.videos.slice() : [];
      ...
      if (uploaded.newMediaPhotos && Array.isArray(uploaded.newMediaPhotos) && uploaded.newMediaPhotos.length > 0) {
        ...
        // 🔥 沒有新媒體照片時，才使用舊系統資料
        existingPhotos = (uploaded.files && Array.isArray(uploaded.files.photos)) ? uploaded.files.photos.slice() : [];
      ...
      // 🔍 [調試] 檢查容器內所有節點
      if (photosPreview) {
        console.log('🔍 [applyExistingRecordToCard] 照片容器節點詳情:', {
          student: student.name,
          總節點數: photosPreview.children.length,
          所有節點類名: Array.prototype.slice.call(photosPreview.children).map(function(n) { return n.className; }),
          newUpload節點: photosPreview.querySelectorAll('.file-preview.new-upload').length,
          existing節點: photosPreview.querySelectorAll('.file-preview.existing, .file-preview.loaded').length
        });
      }
      ...
      if (photosPreview) {
        ...
        if (existingPhotos.length === 0) {
          ...
          allNodes.forEach(function(node) {
            // 只保留正在上傳中的節點（有 .loading 或 .uploading 類別）
            ...
            if (!isUploading) {
              ...
              console.log('📌 [保留] 上傳中照片節點:', node.className);
            }
          });
        } else {
          // 正常情況：只移除舊的已上傳預覽，保留新上傳的預覽
          var oldNodes = Array.prototype.slice.call(photosPreview.querySelectorAll('.file-preview.existing, .file-preview.loaded'));
        ...
        // 🔥 [修復] 只重新插入尚未在伺服器端的照片節點
        // 如果照片已經在 existingPhotos 中，就不重新插入（避免重複顯示）
        var existingPhotoIds = new Set();
        existingPhotos.forEach(function(item) {
          if (typeof item === 'object' && item.id) {
            existingPhotoIds.add(item.id);
          } else if (typeof item === 'string') {
            existingPhotoIds.add(item);
          }
        });
        
        var photoNodesToReinsert = preserveNewPhotoNodes.filter(function(node) {
          var imgEl = node.querySelector('img');
          var blobUrl = node.getAttribute('data-blob-url') || node.getAttribute('data-object-url');
          if (!blobUrl && imgEl) {
            blobUrl = imgEl.getAttribute('src');
          }
          var hasBlobUrl = blobUrl && blobUrl.startsWith('blob:');
          
          // 🔥 [修復] 方法1：檢查是否是「同步中」狀態 → 直接移除（伺服器已有檔案）
          var hasAwaitingPersistence = node.classList.contains('awaiting-persistence');
          var hasUploadSuccess = node.classList.contains('upload-success');
          var hasAwaitingSync = node.getAttribute('data-awaiting-sync') === '1';
          
          if (hasAwaitingPersistence || hasUploadSuccess || hasAwaitingSync) {
            console.log('📌 [照片過濾] 移除「同步中」佔位節點（伺服器已回傳正確檔案）');
            try { 
              revokePreviewObjectUrl(node);
              node.remove(); 
            } catch (e) {}
            return false;  // 已上傳完成，移除佔位框
          }
          
          // 🔥 [修復] 方法2：檢查是否有 Blob URL（本地預覽且還在上傳中）→ 保留
          if (hasBlobUrl) {
            console.log('📌 [照片過濾] 保留 Blob URL 節點:', blobUrl.substring(0, 30));
            return true;  // 本地預覽，保留
          }
          
          // 🔥 方法3：檢查是否正在上傳中（loading 但還沒完成）→ 保留
          var isUploading = node.classList.contains('loading') && 
                           !hasUploadSuccess && 
                           !hasAwaitingPersistence;
          
          if (isUploading) {
            console.log('📌 [照片過濾] 保留上傳中節點');
            return true;
          }
          
          // 🔥 方法4：其他情況 → 移除
          console.log('📌 [照片過濾] 移除未知狀態節點');
          try { node.remove(); } catch (e) {}
          return false;
        });
        
        console.log('🔄 [applyExistingRecordToCard] 重新插入新上傳照片節點:', {
          student: student.name,
          preserveNewPhotoNodes: preserveNewPhotoNodes.length,
          photoNodesToReinsert: photoNodesToReinsert.length,
          existingPhotos: existingPhotos.length,
          existingPhotoIds: Array.from(existingPhotoIds),
          已過濾掉: preserveNewPhotoNodes.length - photoNodesToReinsert.length
        });
        
        photoNodesToReinsert.forEach(function (node, idx) {
          var isInDOM = node.parentNode !== null;
          var className = node.className || '';
          console.log('📌 [applyExistingRecordToCard] 照片節點 ' + idx + ':', {
            className: className,
            isInDOM: isInDOM,
            parentNode: node.parentNode ? node.parentNode.id : 'null',
            hasAwaitingPersistence: node.classList.contains('awaiting-persistence'),
            hasLoading: node.classList.contains('loading')
          });
          
          photosPreview.appendChild(node);
        });
        
        console.log('✅ [applyExistingRecordToCard] 照片節點重新插入完成:', photoNodesToReinsert.length);
      }
      if (videosPreview) {
        // 🔥 [修復1] 先清除所有「同步中」佔位框（無論伺服器返回什麼）
        var awaitingVideoNodes = Array.prototype.slice.call(videosPreview.querySelectorAll('.file-preview.new-upload[data-awaiting-sync="1"]'));
        if (awaitingVideoNodes.length > 0) {
          console.log('🗑️ [applyExistingRecordToCard] 清除影片「同步中」佔位框:', awaitingVideoNodes.length, '個');
          awaitingVideoNodes.forEach(function(node) {
            try {
              revokePreviewObjectUrl(node);
              node.remove();
              console.log('✅ 已移除「同步中」影片節點');
            } catch (e) {
              console.warn('⚠️ 移除「同步中」影片節點失敗:', e);
            }
          });
        }
        ...
        if (existingVideos.length === 0) {
          console.log('🗑️ [applyExistingRecordToCard] 伺服器返回空影片陣列，清除所有非上傳中節點');
          var allNodes = Array.prototype.slice.call(videosPreview.querySelectorAll('.file-preview'));
          allNodes.forEach(function(node) {
            // 只保留正在上傳中的節點（有 .loading 或 .uploading 類別）
            var isUploading = node.classList.contains('loading') || 
                             node.classList.contains('uploading') ||
                             (node.classList.contains('new-upload') && !node.classList.contains('upload-success'));
            if (!isUploading) {
              console.log('🗑️ [清理] 移除舊影片節點:', node.className);
              try { 
                revokePreviewObjectUrl(node);
                node.remove(); 
              } catch (e) {}
            } else {
              console.log('📌 [保留] 上傳中影片節點:', node.className);
            }
          });
        } else {
          // 正常情況：只移除舊的已上傳預覽，保留新上傳的預覽
          Array.prototype.slice.call(videosPreview.querySelectorAll('.file-preview.existing, .file-preview.loaded')).forEach(function(node) {
            try { node.remove(); } catch (e) {}
          });
        }
        ...
        validVideos.forEach(function (item) {
          // 🔥 [修復] 處理新系統影片（物件，使用 id 欄位）或舊系統影片（字串）
          var isNewSystem = (typeof item === 'object' && item.id);
          var filename = isNewSystem ? item.filename : item;
          var videoId = isNewSystem ? item.id : null;
          
          // 建立 URL（新系統使用 video.id，舊系統使用 filename）
          var url;
          if (isNewSystem) {
            // 新系統：使用 /api/media/videos/:id/download
            url = '/api/media/videos/' + videoId + '/download';
            console.log('🎬 [主卡片] 新系統影片下載 URL:', { videoId: videoId, url: url });
          } else {
          ...
          // 縮圖 URL
          var thumbUrl = '';
          var shouldShowPlaceholder = false;  // 🔥 新增：是否顯示占位符
          
          if (isNewSystem) {
            // 🔥 [修復] 與抽屜邏輯一致：只要有 thumbnailFilename 就顯示縮圖
            var videoStatus = item.status || 'unknown';
            // 🔥 [修復] 擴大 isProcessing 定義，涵蓋所有處理中的狀態
            var isProcessing = (
              videoStatus === 'queued' || 
            ...
            if (item.thumbnailFilename) {
              ...
              // 影片正在處理中且沒有縮圖：顯示「處理中」圖標
              shouldShowPlaceholder = true;
              console.log('⏳ [主卡片] 新系統影片 - 顯示處理中圖標:', { 
                videoId: videoId,
                filename: item.filename,
                status: videoStatus,
                '原因': '縮圖尚未生成（status=' + videoStatus + '）'
              });
              
              // 🔥 啟動自動檢查機制（10秒後檢查一次）
              scheduleProcessingVideosCheck();
            } else {
              // 🔥 [修復] 其他情況：不嘗試載入（避免 404），直接顯示預設圖標
              shouldShowPlaceholder = true;
              console.log('⚠️ [主卡片] 新系統影片 - 顯示預設圖標:', { 
                videoId: videoId,
                filename: item.filename,
                thumbnailFilename: item.thumbnailFilename || '(null)',
                status: videoStatus,
                '原因': '沒有縮圖檔名且非處理中狀態（避免 404）'
              });
            }
          } else {
            // 舊系統：從 thumbMap 查找
        ...
        // 🔥 [修復] 只重新插入尚未在伺服器端的影片節點
        // 如果影片已經在 existingVideos 中，就不重新插入（避免重複顯示）
        ...
        var nodesToReinsert = preserveNewVideoNodes.filter(function(node) {
          var videoEl = node.querySelector('video');
          var blobUrl = node.getAttribute('data-blob-url') || node.getAttribute('data-object-url');
          if (!blobUrl && videoEl) {
            blobUrl = videoEl.getAttribute('src');
          }
          var hasBlobUrl = blobUrl && blobUrl.startsWith('blob:');
          ...
          if (hasAwaitingPersistence || hasUploadSuccess || hasAwaitingSync) {
            console.log('📌 [影片過濾] 移除「同步中」佔位節點（伺服器已回傳正確檔案）');
            try { 
              revokePreviewObjectUrl(node);
              node.remove(); 
            } catch (e) {}
          ...
          // 🔥 [修復] 方法2：檢查是否有 Blob URL（本地預覽且還在上傳中）→ 保留
          if (hasBlobUrl) {
            console.log('📌 [影片過濾] 保留 Blob URL 節點:', blobUrl.substring(0, 30));
            return true;  // 本地預覽，保留
          }
          ...
          if (isUploading) {
            console.log('📌 [影片過濾] 保留上傳中節點');
            return true;
          }
          
          // 🔥 方法4：其他情況 → 移除
          console.log('📌 [影片過濾] 移除未知狀態節點');
          try { node.remove(); } catch (e) {}
          return false;
        });
        
        console.log('🔄 [applyExistingRecordToCard] 重新插入新上傳影片節點:', {
          student: student.name,
          preserveNewVideoNodes: preserveNewVideoNodes.length,
          nodesToReinsert: nodesToReinsert.length,
          existingVideos: existingVideos.length,
          existingVideoIds: Array.from(existingVideoIds),
          已過濾掉: preserveNewVideoNodes.length - nodesToReinsert.length
        });
        
        nodesToReinsert.forEach(function (node, idx) {
          console.log('📌 [applyExistingRecordToCard] 影片節點 ' + idx + ':', {
            className: node.className,
            isInDOM: node.parentNode !== null,
            hasAwaitingPersistence: node.classList.contains('awaiting-persistence'),
            hasLoading: node.classList.contains('loading')
          });
        ...
        console.log('✅ [applyExistingRecordToCard] 影片節點重新插入完成:', nodesToReinsert.length);
      }
      // 帶入既存評論（可再編輯）
      ...
      try {
        ...
        if (videosPreview) {
          ...
          if (needPosterVideos.length > 0) {
            ...
            SmartPosterGenerator.processContainer(videosPreview);
          } else {
  ...
  async function handleFileSelect(event, studentIndex, type) {
    var fileInputEl = (event && event.target) ? event.target : null;
    try {
      var ctx = validateStudentSelection(event, studentIndex, type);
      if (!ctx) return;
      var pipelineResult = await runStudentSelectionPipeline(ctx);
      if (pipelineResult && pipelineResult.updated) {
        finalizeStudentSelection(ctx, pipelineResult);
      }
    } catch (error) {
      ...
      resetInputValue(fileInputEl);
    }
  }

  function validateStudentSelection(event, studentIndex, type) {
    ...
    console.log('🔍 [handleFileSelect] 學生', studentIndex, type, '- 接收到', files.length, '個檔案');
  ...
  async function runStudentSelectionPipeline(context) {
    var files = context.files.slice();
    var entry = context.entry;
    var studentIndex = context.studentIndex;
    ...
    if (window.SharedIntegration && typeof window.SharedIntegration.handleStudentMediaSelect === 'function') {
      try {
        ...
        return { updated: true, processedFiles: files, useSharedPreviewer: true };
      } catch (sharedErr) {
        console.warn('⚠️ [handleFileSelect] 共用模組處理失敗，回退到原有邏輯:', sharedErr);
      }
    }

    var pipeline = prepareSelectionPipeline(files, type, context.event);
    if (!pipeline.validFiles.length) {
      console.warn('⚠️ [handleFileSelect] 沒有有效檔案');
      return null;
    }

    var processed = await processFilesSequentially({
      files: pipeline.validFiles,
      type: type,
      entry: entry,
      studentIndex: studentIndex,
      progressToastId: pipeline.progressToastId
    });

    if (pipeline.progressToastId && pipeline.hideProgressFn) {
      pipeline.hideProgressFn(pipeline.progressToastId);
    }

    return {
      updated: true,
      processedFiles: processed.processedFiles || pipeline.validFiles,
      processedCount: processed.processedCount || pipeline.validFiles.length,
      useSharedPreviewer: !!window.SharedMediaPreviewer
    };
  }

  function prepareSelectionPipeline(files, type, eventSource) {
    ...
    files.forEach(function(file) {
      if (!file || !file.name) {
        invalidFiles.push('無效的檔案');
        return;
      }
      var fileType = file.type || '';
      var fileName = file.name.toLowerCase();
      if (type === 'photos') {
        var isImage = allowedImageTypes.indexOf(fileType) >= 0 || /\.(jpe?g|png|webp|gif|heic|heif)$/i.test(fileName);
        if (!isImage) {
          invalidFiles.push(file.name + ' (非圖片格式)');
          return;
        }
      } else if (type === 'videos') {
        var isVideo = allowedVideoTypes.indexOf(fileType) >= 0 || /\.(mp4|webm|mov|avi|quicktime)$/i.test(fileName);
        if (!isVideo) {
          invalidFiles.push(file.name + ' (非影片格式)');
          return;
        }
      }
      var MAX_FILE_SIZE = 100 * 1024 * 1024;
      if (file.size > MAX_FILE_SIZE) {
        invalidFiles.push(file.name + ' (檔案過大: ' + (file.size / 1024 / 1024).toFixed(2) + ' MB)');
        return;
      }
    ...
    if (invalidFiles.length > 0) {
      console.warn('⚠️ [handleFileSelect] 無效檔案:', invalidFiles);
      showToast('部分檔案格式不正確或過大: ' + invalidFiles.slice(0, 3).join(', '), 'warning');
    }
    ...
    if (memoryStatus && memoryStatus.level === 'critical') {
      ...
      if (eventSource && eventSource.target) {
        try { eventSource.target.value = ''; } catch (e) {}
      }
      return { validFiles: [] };
    }
    if (memoryStatus && memoryStatus.level === 'high') {
      var ConfigHigh = window.LearningUploadConfig;
      if (ConfigHigh && ConfigHigh.set) {
        ConfigHigh.set('processing.batchSize', 2);
    ...
    var showProgressFn = window.FLB && window.FLB.UI && window.FLB.UI.showProgressToast;
    var hideProgressFn = window.FLB && window.FLB.UI && window.FLB.UI.hideProgressToast;
    var progressToastId = null;
    if (validFiles.length > 3 && showProgressFn) {
    ...
    return {
      ...
      hideProgressFn: hideProgressFn
    };
  }

  async function processFilesSequentially(options) {
    var files = options.files;
    var type = options.type;
    var entry = options.entry;
    var studentIndex = options.studentIndex;
    var progressToastId = options.progressToastId;
    var updateProgressFn = window.FLB && window.FLB.UI && window.FLB.UI.updateProgressToast;
    ...
    for (var i = 0; i < files.length; i++) {
      try {
        ...
        console.log('📦 [' + (i + 1) + '/' + files.length + '] 處理:', fileName);
        ...
        processedCount++;
        updateSelectionCounters(studentIndex, type);

        if (progressToastId && updateProgressFn) {
          var pct = Math.round((processedCount / files.length) * 100);
          updateProgressFn(progressToastId, pct);
        }

        if (i % 3 === 0 || i === files.length - 1) {
          renderSelectionPreview(studentIndex, type, entry);
        ...
        console.error('❌ [handleFileSelect] 流式處理失敗:', err);
      }
    }

    return { processedFiles: processedFiles, processedCount: processedCount };
  }

  function renderSelectionPreview(studentIndex, type, entry) {
    try {
      if (window.SharedMediaPreviewer) {
        var container = document.getElementById(type + '-preview-' + studentIndex);
        if (container) {
      ...
      console.warn('⚠️ 渲染預覽失敗，回退 updateFilePreviews:', err);
      updateFilePreviews(studentIndex, type);
    }
  }

  function updateSelectionCounters(studentIndex, type) {
  ...
  function finalizeStudentSelection(context, pipelineResult) {
    try { delete lastSubmittedSnapshot[context.studentIndex]; } catch (e) {}
    resetUploadRetryState(context.studentIndex);
    checkUploadReady(context.studentIndex);
    try {
      if (typeof refreshProgress === 'function') refreshProgress();
      if (typeof renderBottomTabs === 'function') renderBottomTabs();
    } catch (err) {
    ...
    try {
      updateDropZones(context.studentIndex);
      appendAddMoreButton(context.studentIndex, 'photos');
      appendAddMoreButton(context.studentIndex, 'videos');
    } catch (uiErr) {
      console.warn('⚠️ 更新投放區狀態失敗:', uiErr);
    }
    if (context.type === 'videos' && context.entry && context.entry.videos && context.entry.videos.length) {
      try { lastSharedVideoFiles = context.entry.videos.slice(-pipelineResult.processedCount || -1); } catch (e) {}
      try { refreshSuggestionChipsAll(); } catch (e) {}
    }
    notifyStudentStateChange(context.studentIndex);
  }
  ...
  function updateFilePreviews(studentIndex, type) {
    ...
    var existingNodes = Array.prototype.slice.call(previewContainer.querySelectorAll('.file-preview.existing, .file-preview.loaded'));
    console.log('💾 [updateFilePreviews] 保存', existingNodes.length, '個已上傳的預覽節點');
    ...
    filesToRenderNow.forEach(function (file, fileIndex) {
      ...
      removeBtn.onclick = function() { return removeFile(studentIndex, type, fileIndex); };
      var removeIcon = document.createElement('i');
      removeIcon.className = 'fas fa-times';
      removeBtn.appendChild(removeIcon);
      previewDiv.appendChild(removeBtn);
      
      var overlay = createSharedOverlay('待上傳');
      previewDiv.appendChild(overlay);
      
      var progressPair = createSharedProgress();
      previewDiv.appendChild(progressPair.root);
      var progressFill = progressPair.fill;
    ...
    if (type === 'videos') {
      try {
        SmartPosterGenerator.processContainer(previewContainer);
      } catch (e) {
    ...
    if (filesToRenderLater.length > 0) {
      ...
      // 使用 requestIdleCallback 或降級為 setTimeout
      if ('requestIdleCallback' in window) {
        requestIdleCallback(renderDeferredPreviews, { timeout: 2000 });
      } else {
        setTimeout(renderDeferredPreviews, 100);
      }
    }
  }
  ...
  function updateDropZones(index) {
    try {
      ...
      if (photoZone) {
        var hidePhotoZone = tp > 0;
        photoZone.style.display = hidePhotoZone ? 'none' : '';
      }
      if (videoZone) {
        var hideVideoZone = tv > 0;
        videoZone.style.display = hideVideoZone ? 'none' : '';
      }
    } catch (e) {}
  }
  ...
  function appendAddMoreButton(index, type) {
    ...
    var tp = getTotalCount(index, 'photos');
    var tv = getTotalCount(index, 'videos');
    var shouldShowButton = type === 'photos' ? tp > 0 : tv > 0;
  ...
  function createSharedOverlay(text) {
    if (window.FLBSharedUI && typeof window.FLBSharedUI.createOverlay === 'function') {
      return window.FLBSharedUI.createOverlay({ text: text || '處理中' });
    }
    var overlay = document.createElement('div');
    overlay.className = 'file-uploading-overlay';
    var span = document.createElement('span');
    span.className = 'progress-text';
    span.textContent = text || '處理中';
    overlay.appendChild(span);
    return overlay;
  }

  function createSharedProgress() {
    if (window.FLBSharedUI && typeof window.FLBSharedUI.createProgressBar === 'function') {
      return window.FLBSharedUI.createProgressBar();
    }
    var root = document.createElement('div');
    root.className = 'file-upload-progress';
  ...
  function removeFile(studentIndex, type, fileIndex) {
    ...
    // 🔄 即時同步至抽屜/已上傳記錄
    notifyStudentStateChange(studentIndex);
    try {
      if (window.FLB && FLB.State) {
  ...
  function notifyStudentStateChange(index) {
    try {
      if (window.LearningUploadStudentStore && typeof window.LearningUploadStudentStore.set === 'function') {
        window.LearningUploadStudentStore.set(index, studentFiles[index]);
      }
    } catch (e) {}
  }
    ...
    try {
          ...
          return new Promise(function (resolve, reject) {
            ...
            try {
              ...
              var metadata = {
                ...
                instructorName: currentCourse.instructor || '',  // ✅ 使用行事曆的講師資訊
                dateKey: dateValue,
                mode: 'student'
              };
                  ...
                  if (type === 'videos') {
                    ...
                    try {
                      if (stu && stu.uploaded) {
                        ...
                        // 🔥 [修復] 清除对应的本地预览（使用多種選擇器）
                        try {
                          var previewId = 'preview-video-' + idx;
                          var localPreview = document.getElementById(previewId);
                          
                          // 如果透過 ID 找不到，嘗試透過容器選擇器找
                          if (!localPreview) {
                            var container = document.getElementById('videos-preview-' + studentIndex);
                            if (container) {
                              var videosPreviews = container.querySelectorAll('.file-preview.new-upload');
                              if (videosPreviews[idx]) {
                                localPreview = videosPreviews[idx];
                              }
                            }
                          }
                          
                          if (localPreview) {
                            console.log('🧹 清除本地影片预览:', previewId);
                            revokePreviewObjectUrl(localPreview);
                            localPreview.remove();
                          } else {
                            console.warn('⚠️ 找不到本地预览:', previewId);
                          }
                        } catch (e) {
                    ...
                    try {
                      if (stu && stu.uploaded) {
                        ...
                        // 🔥 [修復] 清除对应的本地预览（使用多種選擇器）
                        try {
                          var previewId = 'preview-image-' + idx;
                          var localPreview = document.getElementById(previewId);
                          
                          // 如果透過 ID 找不到，嘗試透過容器選擇器找
                          if (!localPreview) {
                            var container = document.getElementById('photos-preview-' + studentIndex);
                            if (container) {
                              var photosPreviews = container.querySelectorAll('.file-preview.new-upload');
                              if (photosPreviews[idx]) {
                                localPreview = photosPreviews[idx];
                              }
                            }
                          }
                          
                          if (localPreview) {
                            console.log('🧹 清除本地照片预览:', previewId);
                            revokePreviewObjectUrl(localPreview);
                            localPreview.remove();
                          } else {
                            console.warn('⚠️ 找不到本地预览:', previewId);
          ...
          function attempt(n) {
            return new Promise(function (resolve, reject) {
              ...
              try {
                ...
                var doReq = function () {
                  // ⚠️ 使用旧 API 作为备用路径（小文件或分片上传失败时）
                  console.warn('⚠️ [向後兼容] 學生檔案使用舊 API 上傳:', file.name, '大小:', ChunkedUploader.formatFileSize(file.size));
      ...
      function runWithLimit(taskFns, limit) {
        ...
        if (includeComment) {
          ...
          // 🆕 使用新 API v2 (纯文字评语保存)
          console.log('📝 [v2] 保存評語:', student.name);
          if (commentFill) commentFill.style.width = '50%';
          if (commentText) commentText.textContent = '準備中';
          ...
          commentPromise = fetch('/api/learning-records/save', {
            ...
            body: JSON.stringify({
              ...
              relativePath: opMeta.relativePathUnified,
              instructorName: currentCourse.instructor || '',  // 🔥 新增：講師資訊
              mediaIds: []  // 媒体已单独上传
            })
          }).then(function(resp) {
            if (commentFill) commentFill.style.width = '100%';
            if (commentText) commentText.textContent = '已儲存';
            updateOverallButtonProgress();
            try { UploadCenter.update(studentIndex, 'comment', -1, 100); } catch (e) {}
            return resp.json();
          }).then(function(data) {
            ...
            console.error('❌ 評語保存失敗:', err);
            if (commentText) commentText.textContent = '失敗';
            throw err;
          }).finally(function(){ 
            try { decActiveUpload(studentIndex); } catch (e) {} 
          });
        ...
        // 🔍 詳細記錄失敗的檔案
        if (!allOk) {
          console.error('❌❌❌ [uploadStudentRecord] 有檔案上傳失敗！');
          console.error('📊 照片上傳結果（前 3 張）:', resultsPhotosPrimary.map(function(r, i) {
            return {
              index: i,
              status: r.status,
              error: r.status === 'rejected' ? (r.reason && r.reason.message || r.reason) : null
            };
          }));
          console.error('📊 照片上傳結果（剩餘）:', resultsPhotosRest.map(function(r, i) {
            return {
              index: i + resultsPhotosPrimary.length,
              status: r.status,
              error: r.status === 'rejected' ? (r.reason && r.reason.message || r.reason) : null
            };
          }));
          console.error('📊 影片上傳結果:', resultsVideos.map(function(r, i) {
            return {
              index: i,
              status: r.status,
              error: r.status === 'rejected' ? (r.reason && r.reason.message || r.reason) : null
        ...
        if (allOk) {
          ...
          // 🔥 [修復] 直接移除該學生的所有上傳任務（不等待 800ms）
          try {
            var tasksToRemove = [];
            (pendingPhotos || []).forEach(function (_, idx) { tasksToRemove.push({ type: 'photos', idx: idx }); });
            (pendingVideos || []).forEach(function (_, idx) { tasksToRemove.push({ type: 'videos', idx: idx }); });
            if (includeComment) { tasksToRemove.push({ type: 'comment', idx: -1 }); }
            
            tasksToRemove.forEach(function(task) {
              var sid = String(studentIndex) + '-' + String(task.type) + '-' + String(task.idx);
              var taskIdx = UploadCenter.tasks.findIndex(function (x) { return x.sid === sid; });
              if (taskIdx > -1) {
                UploadCenter.tasks.splice(taskIdx, 1);
              }
            });
            UploadCenter.render();
            console.log('✅ [uploadStudentRecord] 已清空上傳佇列:', student.name);
          } catch (e) {
            console.warn('⚠️ [uploadStudentRecord] 清空佇列失敗:', e);
          }
        } else {
          btn.classList.add('error');
          btn.disabled = false;
        ...
        // 🔥 立即重新整理（無延遲）+ 顯示視覺回饋
        console.log('🔄 [上傳完成] 重新整理學習記錄列表...');
        try { 
          showToast('正在載入最新記錄...', 'info');
          await loadUploadedRecordsForCurrentCourse({ force: true });
          showToast('記錄已更新', 'success');
        } catch (e) {
          console.warn('⚠️ 重新整理失敗:', e);
        }
        // 🔁 延遲再次檢查（確保 NAS 寫入完成）
        try { setTimeout(function(){ try { fetchStudentFsRecord(studentIndex, student); } catch (e) {} }, 500); } catch (e) {}
        
        // 🔥 新增：靜默輪詢檢查影片縮圖（最多檢查 5 次，每次間隔 3 秒）
        try {
          var videoCheckCount = 0;
          var maxVideoChecks = 5;
          var videoCheckInterval = 3000;  // 增加間隔到 3 秒
          
          var checkVideoThumbnails = function() {
            videoCheckCount++;
            if (videoCheckCount > maxVideoChecks) {
              console.log('⏱️ [影片縮圖檢查] 已達到最大檢查次數，停止輪詢');
              return;
            }
            
            console.log('🔍 [影片縮圖檢查-靜默] 第', videoCheckCount, '次檢查學生:', student.name);
            
            // 🔥 使用靜默更新，不重繪整個卡片
            silentCheckStudentVideoThumbnails(studentIndex, student.name).then(function(updated) {
              if (updated) {
                console.log('✅ [縮圖已更新] 停止輪詢');
                return;  // 縮圖已更新，停止輪詢
              }
              
              // 繼續檢查
              if (videoCheckCount < maxVideoChecks) {
                setTimeout(checkVideoThumbnails, videoCheckInterval);
              }
            });
          };
          
          // 首次檢查在 3 秒後開始
          setTimeout(checkVideoThumbnails, 3000);
        } catch (e) {
          console.warn('⚠️ 啟動影片縮圖檢查失敗:', e);
        }
      ...
      // ⚠️ 批次上傳備用路徑（僅在禁用分片上傳時使用）
      console.warn('⚠️ [向後兼容] 批次上傳使用舊 API:', pendingPhotos.length, '張照片,', pendingVideos.length, '支影片');
      ...
      // 🔥 [修復] 直接移除該學生的批次上傳任務
      try {
        var sid = String(studentIndex) + '-batch-0';
        var taskIdx = UploadCenter.tasks.findIndex(function (x) { return x.sid === sid; });
        if (taskIdx > -1) {
          UploadCenter.tasks.splice(taskIdx, 1);
          UploadCenter.render();
        }
        console.log('✅ [批次上傳] 已清空上傳佇列:', student.name);
      } catch (e) {
        console.warn('⚠️ [批次上傳] 清空佇列失敗:', e);
      }
      ...
      // 🔥 立即重新整理（批次上傳完成）
      console.log('🔄 [批次上傳完成] 重新整理學習記錄列表...');
      try { 
        await loadUploadedRecordsForCurrentCourse({ force: true });
        showToast('批次記錄已更新', 'success');
      } catch (e) {
        console.warn('⚠️ 重新整理失敗:', e);
      }
      // 針對當前學生再觸發一次單點查詢，優先讓本卡片取得正式檔名與縮圖（延遲減少）
      try { setTimeout(function(){ try { fetchStudentFsRecord(studentIndex, student); } catch (e) {} }, 500); } catch (e) {}
      
      // 🔥 新增：靜默輪詢檢查照片縮圖（批次上傳）
      try {
        var photoCheckCount = 0;
        var maxPhotoChecks = 5;
        var photoCheckInterval = 2000;  // 照片生成較快，間隔 2 秒
        
        var checkPhotoThumbnails = function() {
          photoCheckCount++;
          if (photoCheckCount > maxPhotoChecks) {
            console.log('⏱️ [批次-照片縮圖檢查] 已達到最大檢查次數，停止輪詢');
            return;
          }
          
          console.log('🔍 [批次-照片縮圖檢查-靜默] 第', photoCheckCount, '次檢查學生:', student.name);
          
          // 🔥 使用靜默更新，不重繪整個卡片
          silentCheckStudentPhotoThumbnails(studentIndex, student.name).then(function(updated) {
            if (updated) {
              console.log('✅ [批次-照片縮圖已更新]');
            }
            
            // 繼續輪詢直到達到最大次數（因為可能有多張照片陸續生成）
            if (photoCheckCount < maxPhotoChecks) {
              setTimeout(checkPhotoThumbnails, photoCheckInterval);
            }
          });
        };
        
        // 照片生成通常較快，延遲 1 秒後開始第一次檢查
        setTimeout(checkPhotoThumbnails, 1000);
      } catch (e) {
        console.warn('⚠️ 啟動批次照片縮圖檢查失敗:', e);
      }
      ...
      try {
        ...
        var checkVideoThumbnails = function() {
          videoCheckCount++;
          if (videoCheckCount > maxVideoChecks) {
            console.log('⏱️ [批次-影片縮圖檢查] 已達到最大檢查次數，停止輪詢');
            return;
          }
          
          console.log('🔍 [批次-影片縮圖檢查-靜默] 第', videoCheckCount, '次檢查學生:', student.name);
          
          // 🔥 使用靜默更新，不重繪整個卡片
          silentCheckStudentVideoThumbnails(studentIndex, student.name).then(function(updated) {
            if (updated) {
              console.log('✅ [批次-影片縮圖已更新] 停止輪詢');
              return;
            }
  ...
  async function uploadOverview(options) {
    console.log('🚀🚀🚀 [uploadOverview] 函數被調用！options:', options);
    console.log('📊 [uploadOverview] 當前課程:', currentCourse);
    ...
    // 🔥 重要：fileProgressMap 必須在最前面定義（在任何上傳調用之前）
    var fileProgressMap = {};
    console.log('✅ [uploadOverview] fileProgressMap 已初始化');
    ...
    // 🔥 【重要】企業級上傳函數必須在此定義（在任何上傳調用之前）
    // ============================================
    // 企業級分片上傳處理函數
    // ============================================
    
    function uploadOneOverviewChunked(kind, file, idx, withSummary, displayIndex) {
      console.log('🔥 [uploadOneOverviewChunked] 開始執行, fileProgressMap 存在?', typeof fileProgressMap !== 'undefined');
      
      return new Promise(function(resolve, reject) {
        var key = 'overview-' + kind + '-' + idx;
        
        // 🔍 除錯：確認 fileProgressMap 存在
        if (typeof fileProgressMap === 'undefined') {
          console.error('❌ [uploadOneOverviewChunked] fileProgressMap 未定義！作用域錯誤！');
          reject(new Error('fileProgressMap 未定義 - 作用域錯誤'));
          return;
        }
        
        fileProgressMap[key] = 0;
        console.log('✅ [uploadOneOverviewChunked] fileProgressMap[' + key + '] = 0 設置成功');
        ...
        try {
          ...
          var metadata = {
            ...
            instructorName: currentCourse.instructor || parsed.instructor || '',
            dateKey: dateStr,
            mode: 'overview'
          };
          ...
          // 🔥 重要：總是添加 relativePath
          if (metaPath && metaPath.relativePathUnified) {
            metadata.relativePath = metaPath.relativePathUnified;
            console.log('✅ [分片上傳] 添加 relativePath:', metadata.relativePath);
          }
          ...
          if (kind === 'image' && window.PhotoPreprocessor && file.type && file.type.startsWith('image/')) {
            console.log('🖼️ [分片上傳] 壓縮照片:', file.name);
            compressionPromise = window.PhotoPreprocessor.compressPhoto(file, {
              maxWidth: 1920,
              maxHeight: 1920,
              quality: 0.85
              ...
              if (result.blob && result.compressedSize < file.size) {
                fileToUpload = new File([result.blob], file.name, { type: result.blob.type });
                console.log('✅ [分片上傳] 壓縮成功，節省:', result.savings + '%');
              }
            }).catch(function(err) {
              console.warn('⚠️ [分片上傳] 壓縮失敗，使用原檔:', err);
            });
          }
          ...
          compressionPromise.then(function() {
            ...
            console.error('❌ [分片上傳] 壓縮失敗:', compressionErr);
            try {
    ...
    function uploadOne(kind, file, idx, withSummary, displayIndex){
      console.log('🔥 [uploadOne] 開始執行:', { kind, fileName: file?.name, idx, fileProgressMap: typeof fileProgressMap });
      
      // 🔥 企業級：大檔案自動使用分片上傳
      if (window.ChunkedUploader && ChunkedUploader.shouldUseChunkedUpload(file)) {
        console.log('📦 [企業級上傳] 使用分片上傳:', file.name, ChunkedUploader.formatFileSize(file.size));
        return uploadOneOverviewChunked(kind, file, idx, withSummary, displayIndex);
      }

      return new Promise(function(resolve, reject){
        try {
          console.log('🔥 [uploadOne] Promise 內部, fileProgressMap 存在?', typeof fileProgressMap !== 'undefined');
          
          var fd = buildBaseForm();
          console.log('✅ [uploadOne] buildBaseForm 完成');
          
          if (withSummary) {
            fd.append('overviewSummary', autoBlock);
            console.log('✅ [uploadOne] 已附加 overviewSummary');
          }
          
          if (kind === 'image') {
            console.log('🖼️ [uploadOne-照片] 準備附加照片:', file?.name, '大小:', file?.size);
            try { 
              fd.append('overviewPhotos', file, (file && file.name) ? file.name : 'overview.jpg'); 
              console.log('✅ [uploadOne-照片] FormData.append 成功');
            }
            catch (e) { 
              console.error('❌ [uploadOne-照片] FormData.append 失敗（帶檔名）:', e);
              fd.append('overviewPhotos', file); 
              console.log('🔄 [uploadOne-照片] FormData.append 成功（無檔名）');
            }
          } else {
            console.log('🎬 [uploadOne-影片] 準備附加影片:', file?.name, '大小:', file?.size);
            try { 
              fd.append('videos', file, (file && file.name) ? file.name : 'overview.webm'); 
              console.log('✅ [uploadOne-影片] FormData.append 成功');
            }
            catch (e2) { 
              console.error('❌ [uploadOne-影片] FormData.append 失敗（帶檔名）:', e2);
              fd.append('videos', file); 
              console.log('🔄 [uploadOne-影片] FormData.append 成功（無檔名）');
            }
          }
          ...
          UploadBudget.acquire((file && file.size) || 1).then(function(tok){
            // ⚠️ 使用旧 API 作为备用路径（小文件或分片上传失败时）
            console.warn('⚠️ [向後兼容] 課程總覽檔案使用舊 API 上傳:', file.name, '大小:', file.size);
            console.log('🔍 [uploadOne] 進度回調前, fileProgressMap:', typeof fileProgressMap);
            var req = global.FLB.Api.uploadRecordWithProgress(fd, function(p){
              console.log('📊 [uploadOne] 進度回調執行, fileProgressMap:', typeof fileProgressMap, 'idx:', idx);
              if (typeof fileProgressMap === 'undefined') {
                console.error('❌ [uploadOne] fileProgressMap 在進度回調中未定義！');
                return;
              }
            ...
            req.promise.then(function(r){ 
              console.log('✅ [uploadOne-' + kind + '] 上傳成功:', file?.name, '回應:', r); 
              try { UploadCenter.done(-1, 'overview', idx); } catch (e) {} 
              try { setOverviewPreviewProgress(kind, displayIndex, 100, 'done'); } catch (e) {} 
              resolve(r); 
            })
              .catch(function(err){ 
                console.error('❌ [uploadOne-' + kind + '] 上傳失敗:', file?.name, '錯誤:', err); 
                console.error('❌ [uploadOne-' + kind + '] 錯誤詳情:', err.message, err.stack); 
                try { UploadCenter.fail(-1, 'overview', idx, String(err && err.message || '失敗')); } catch (e) {} 
                try { setOverviewPreviewProgress(kind, displayIndex, 0, 'error'); } catch (e) {} 
                reject(err); 
              })
    ...
    // 驗證必要欄位
    if (!currentCourse.date && !currentCourse.formattedDate) {
      console.error('❌ 課程缺少日期資訊:', currentCourse);
      showToast('課程日期資訊不完整，無法上傳', 'error');
      return;
    }
    ...
    // 🔍 除錯：輸出 metaPath
    console.log('📁 [uploadOverview] metaPath:', {
      coursePeriod: metaPath && metaPath.coursePeriod,
      relativePathUnified: metaPath && metaPath.relativePathUnified,
      date: metaPath && metaPath.date,
      '完整物件': metaPath
    });
    
    // 🔥 安全地處理日期（考慮虛擬課程可能沒有有效的 start）
    ...
    // 避免重複上傳：若內容與上次上傳快照相同，直接略過（除非強制上傳）
    var skipDuplicateCheck = (options && options.force) || forceNextOverviewUpload;
    console.log('🔍 [uploadOverview] 重複檢查:', {
      'skipDuplicateCheck': skipDuplicateCheck,
      'options.force': options && options.force,
      'forceNextOverviewUpload': forceNextOverviewUpload
    });
    
    if (!skipDuplicateCheck) {
      try { 
        var snapNow = computeOverviewSnapshot(); 
        console.log('📸 [uploadOverview] 快照比對:', {
          '當前快照': snapNow ? snapNow.substring(0, 50) + '...' : null,
          '上次快照': lastOverviewSnapshot ? lastOverviewSnapshot.substring(0, 50) + '...' : null,
          '是否相同': snapNow === lastOverviewSnapshot
        });
        
        if (snapNow === lastOverviewSnapshot) { 
          console.warn('⏭️ [uploadOverview] 內容未變更，跳過上傳（可能導致單張照片無法上傳！）');
          showToast('內容未變更，無需重複上傳', 'info');
          return; 
        } 
      } catch (e) {
        console.error('❌ [uploadOverview] 快照計算失敗:', e);
      }
    } else {
      console.log('🔥 [uploadOverview] 強制上傳模式（跳過重複檢查）');
      forceNextOverviewUpload = false; // 重置標記
    }

    // 資料塊（overviewSummary）
    var autoBlock = buildOverviewBlockFromFields();

    // 🚀 流式處理：圖片/影片壓縮（避免記憶體爆炸）
    console.log('📁 [uploadOverview] 獲取檔案:', {
      '總檔案數': allFilesAll.length,
      '檔案列表': allFilesAll.map(function(f) { return f.name + ' (' + f.type + ')'; })
    });
    
    var allImages = allFilesAll.filter(function(f){ return /^image\//i.test(f.type); });
    var allVideos = allFilesAll.filter(function(f){ return /^video\//i.test(f.type); });
    var totalMediaCount = allImages.length + allVideos.length;
    
    console.log('📊 [uploadOverview] 分類結果:', {
      '照片數量': allImages.length,
      '影片數量': allVideos.length,
      '總媒體數量': totalMediaCount,
      '照片檔名': allImages.map(function(f) { return f.name; }),
      '影片檔名': allVideos.map(function(f) { return f.name; })
    });
    
    // 🔥 手機端記憶體檢查（防止崩潰）
    if (typeof window.__checkMemoryPressure === 'function') {
      try {
        var memStatus = window.__checkMemoryPressure();
        console.log('🧠 [uploadOverview] 記憶體狀態:', memStatus.level, '(' + (memStatus.ratio * 100).toFixed(1) + '%)');
        
        if (memStatus.level === 'critical' || memStatus.level === 'high') {
          console.warn('⚠️ [uploadOverview] 記憶體壓力過高，執行緊急清理...');
          if (typeof window.__emergencyMemoryCleanup === 'function') {
            window.__emergencyMemoryCleanup();
          }
          ...
          if (totalMediaCount > 5) {
            showToast('記憶體不足，建議一次上傳不超過 5 個檔案', 'warning');
          }
        }
      } catch (e) {
        console.warn('⚠️ [uploadOverview] 記憶體檢查失敗:', e);
      }
    }
    ...
    // 顯示進度 Toast（大量檔案時）
    var mediaProgressToastId = null;
    var showProgressFn = window.FLB && window.FLB.UI && window.FLB.UI.showProgressToast;
    var updateProgressFn = window.FLB && window.FLB.UI && window.FLB.UI.updateProgressToast;
    var hideProgressFn = window.FLB && window.FLB.UI && window.FLB.UI.hideProgressToast;
    
    if (totalMediaCount > 5 && showProgressFn) {
      mediaProgressToastId = showProgressFn('處理課程媒體', 0);
    }
    
    var processedMediaCount = 0;
    
    // 🔥 [流式上傳] 建立上傳佇列（壓縮完立即上傳，不等待全部完成）
    var uploadQueue = [];
    var uploadedCount = 0;
    var totalToUpload = allImages.length + allVideos.length;
    
    console.log('🚀 [流式上傳] 開始流式處理與上傳：照片', allImages.length, '個，影片', allVideos.length, '個');
    ...
    console.log('📱 [照片處理] 裝置類型:', isMobile ? '手機端（輕量模式）' : '桌面端（標準模式）');
    
    // 壓縮並立即上傳照片
    for (var i = 0; i < allImages.length; i++) {
      try {
        console.log('🖼️ [照片-開始] 索引:', i, '檔名:', allImages[i].name, '大小:', (allImages[i].size / 1024 / 1024).toFixed(2), 'MB');
        
        // 🔥 手機端：使用更寬鬆的壓縮策略（避免記憶體溢出）
        var processedImg;
        if (isMobile && allImages[i].size < 5 * 1024 * 1024) {
          // 手機端：小於 5MB 的照片直接上傳，不壓縮
          console.log('📱 [照片-手機端] 檔案小於 5MB，跳過壓縮');
          processedImg = allImages[i];
        } else {
          console.log('💻 [照片-壓縮] 開始壓縮...');
          processedImg = await compressImageIfNeeded(allImages[i]);
          console.log('✅ [照片-壓縮完成] 壓縮後大小:', (processedImg.size / 1024 / 1024).toFixed(2), 'MB');
        }
        
        images.push(processedImg);
        
        // 🔥 立即上傳（不等待）
        // uploadOne(kind, file, idx, withSummary, displayIndex)
        // idx: 用於 UploadCenter 追蹤
        // withSummary: 是否為第一個檔案（需附帶課程總覽文字）
        // displayIndex: 用於 UI 預覽更新
        var isFirstFile = (i === 0 && allVideos.length === 0);
        console.log('🚀 [照片-準備上傳] 索引:', i, 'isFirstFile:', isFirstFile, 'displayIndex:', i);
        
        var uploadPromise = uploadOne('image', processedImg, i, isFirstFile, i);
        uploadQueue.push(uploadPromise);
        uploadedCount++;
        
        console.log('📤 [流式上傳] 照片', (i + 1), '/', allImages.length, '已壓縮並開始上傳');
        
      } catch (imgErr) {
        console.error('❌ [照片-處理錯誤] 索引:', i, '錯誤:', imgErr);
        console.error('❌ [照片-錯誤堆疊]', imgErr.stack);
        console.warn('⚠️ 照片壓縮失敗，使用原檔案:', allImages[i].name);
        images.push(allImages[i]);
        
        // 即使失敗也上傳原檔案
        var isFirstFile = (i === 0 && allVideos.length === 0);
        console.log('🔄 [照片-備用上傳] 使用原檔案:', allImages[i].name);
        
        var uploadPromise = uploadOne('image', allImages[i], i, isFirstFile, i);
        uploadQueue.push(uploadPromise);
        uploadedCount++;
      }
      
      processedMediaCount++;
      if (mediaProgressToastId && updateProgressFn) {
        var pct = Math.round((processedMediaCount / totalMediaCount) * 100);
        updateProgressFn(mediaProgressToastId, pct);
      }
    ...
    console.log('📱 [影片處理] 裝置類型:', isMobile ? '手機端（輕量模式）' : '桌面端（標準模式）');
    
    // 壓縮並立即上傳影片
    for (var j = 0; j < allVideos.length; j++) {
      try {
        console.log('🎬 [影片-開始] 索引:', j, '檔名:', allVideos[j].name, '大小:', (allVideos[j].size / 1024 / 1024).toFixed(2), 'MB');
        
        // 🔥 手機端：跳過壓縮，直接上傳原檔案（避免記憶體溢出）
        var processedVid;
        if (isMobile) {
          console.log('📱 [影片-手機端] 跳過壓縮，直接使用原檔案（防止崩潰）');
          processedVid = allVideos[j]; // 不壓縮
        } else {
          console.log('💻 [影片-桌面端] 嘗試壓縮...');
          processedVid = await compressVideoIfNeeded(allVideos[j]);
          console.log('✅ [影片-壓縮完成] 壓縮後大小:', (processedVid.size / 1024 / 1024).toFixed(2), 'MB');
        }
        
        videos.push(processedVid);
        
        // 🔥 立即上傳（不等待）
        // uploadOne(kind, file, idx, withSummary, displayIndex)
        // idx: 影片索引從照片數後開始（避免與照片 idx 重疊）
        // withSummary: 是否為第一個檔案（僅當沒有照片且是第一個影片時）
        // displayIndex: 用於 UI 預覽更新（影片的顯示索引）
        var videoIdx = allImages.length + j; // 影片在整體檔案列表中的索引
        var isFirstFile = (allImages.length === 0 && j === 0);
        
        console.log('🚀 [影片-準備上傳] 索引:', videoIdx, 'isFirstFile:', isFirstFile, 'displayIndex:', j);
        
        var uploadPromise = uploadOne('video', processedVid, videoIdx, isFirstFile, j);
        uploadQueue.push(uploadPromise);
        uploadedCount++;
        
        console.log('📤 [流式上傳] 影片', (j + 1), '/', allVideos.length, '已處理並開始上傳');
        
      } catch (vidErr) {
        console.error('❌ [影片-處理錯誤] 索引:', j, '錯誤:', vidErr);
        console.error('❌ [影片-錯誤堆疊]', vidErr.stack);
        console.warn('⚠️ 影片處理失敗，使用原檔案:', allVideos[j].name);
        videos.push(allVideos[j]);
        
        // 即使失敗也上傳原檔案
        var videoIdx = allImages.length + j;
        var isFirstFile = (allImages.length === 0 && j === 0);
        console.log('🔄 [影片-備用上傳] 使用原檔案:', allVideos[j].name);
        
        var uploadPromise = uploadOne('video', allVideos[j], videoIdx, isFirstFile, j);
        uploadQueue.push(uploadPromise);
        uploadedCount++;
      }
    ...
    // 隱藏進度 Toast
    if (mediaProgressToastId && hideProgressFn) {
      hideProgressFn(mediaProgressToastId);
    }
    
    console.log('✅ [流式上傳] 所有檔案已壓縮並開始上傳：照片', images.length, '個，影片', videos.length, '個');
    console.log('⏳ [流式上傳] 等待', uploadQueue.length, '個上傳任務完成...');
    ...
    // ✅ fileProgressMap 已在函數開頭定義，確保所有上傳函數都能訪問
    function buildBaseForm(){
      var fd = new FormData();
      // 🔥 優先使用課程物件的欄位（虛擬課程已包含必要欄位）
      ...
      console.log('📝 [uploadOverview] 建立 FormData:', {
        course: course,
        period: period,
        coursePeriod: coursePeriod,
        date: dateStr,
      ...
      // 🔥 coursePeriod 優先順序：coursePeriod > metaPath.coursePeriod
      if (coursePeriod) {
        fd.append('coursePeriod', coursePeriod);
      } else if (metaPath && metaPath.coursePeriod) {
        fd.append('coursePeriod', metaPath.coursePeriod);
      }
      
      // 🔥 重要：總是使用 relativePathUnified（包含日期+主題）
      if (metaPath && metaPath.relativePathUnified) {
        console.log('✅ [uploadOverview] 使用 relativePath:', metaPath.relativePathUnified);
        fd.append('relativePath', metaPath.relativePathUnified);
      } else {
        console.warn('⚠️ [uploadOverview] 缺少 relativePath，將使用預設路徑（可能導致純日期資料夾）');
      }
    ...
    try {
      ...
      if (!images.length && !videos.length && autoBlock && autoBlock.trim().length) {
        ...
        // 🆕 使用新 API v2 (纯文字总览保存)
        console.log('📝 [v2] 保存課程總覽（無媒體）');
        if (!silent && btn) btn.innerHTML = '<span class="loading"></span> 儲存中 50%';
        try { if (syncFill) syncFill.style.width = '50%'; if (syncText) syncText.textContent = '同步中 50%'; } catch (e) {}
        ...
        var reqOnly = fetch('/api/learning-records/save', {
          ...
          body: JSON.stringify({
            ...
            coursePeriod: (metaPath && metaPath.coursePeriod) || null,
            relativePath: (metaPath && metaPath.relativePathUnified) || null,
            overviewFolder: '課程總覽',
            instructorName: currentCourse.instructor || parsed.instructor || '',  // 🔥 新增：講師資訊
            mediaIds: []  // 無檔案
          })
        }).then(function(resp) {
          if (!silent && btn) btn.innerHTML = '<span class="loading"></span> 儲存中 100%';
          try { if (syncFill) syncFill.style.width = '100%'; if (syncText) syncText.textContent = '同步中 100%'; } catch (e) {}
          try { if (t) UploadCenter.update(-1, 'overview', t.fileIndex || 0, 100); } catch (e) {}
          return resp.json();
        }).then(function(data) {
          if (!data.success) throw new Error(data.message || '保存失敗');
          try { if (t) UploadCenter.done(-1, 'overview', t.fileIndex || 0); } catch (e) {}
          return data;
        }).catch(function(err) {
          console.error('❌ 課程總覽保存失敗:', err);
          try { if (t) UploadCenter.fail(-1, 'overview', t.fileIndex || 0, String(err && err.message || '失敗')); } catch (e) {}
          throw err;
        });
      ...
      // 🔥 [流式上傳] 等待所有上傳任務完成（已在壓縮時開始上傳）
      var uploadResults = await Promise.allSettled(uploadQueue);
      var failed = uploadResults.some(function(x){ return x && x.status==='rejected'; });
      
      console.log('✅ [流式上傳] 所有上傳任務完成：', {
        total: uploadResults.length,
        成功: uploadResults.filter(function(x){ return x.status === 'fulfilled'; }).length,
        失敗: uploadResults.filter(function(x){ return x.status === 'rejected'; }).length
      });
      
      // 🔍 詳細記錄失敗的檔案
      if (failed) {
        console.error('❌❌❌ [uploadOverview] 有檔案上傳失敗！');
        console.error('📊 上傳結果詳情:');
        uploadResults.forEach(function(result, index) {
          if (result.status === 'rejected') {
            console.error('  ❌ 檔案 #' + index + ':', {
              status: 'rejected',
              error: result.reason && result.reason.message || result.reason,
              stack: result.reason && result.reason.stack
            });
          } else {
            console.log('  ✅ 檔案 #' + index + ':', { status: 'fulfilled' });
          }
        });
        console.error('📁 檔案清單:');
        images.forEach(function(img, i) {
          console.error('  照片 #' + i + ':', img.name, '(' + (img.size / 1024 / 1024).toFixed(2) + ' MB)');
        });
        videos.forEach(function(vid, i) {
          console.error('  影片 #' + i + ':', vid.name, '(' + (vid.size / 1024 / 1024).toFixed(2) + ' MB)');
        });
      }
      ...
      if (!failed) {
        try {
          UploadCenter.tasks = UploadCenter.tasks.filter(function(task) {
            return !(task.studentIndex === -1 && task.type === 'overview');
          });
          UploadCenter.render();
          console.log('✅ [課程總覽] 已清空上傳佇列');
        } catch (e) {
          console.warn('⚠️ [課程總覽] 清空佇列失敗:', e);
        }
      }
      ...
      // 🔥 [修復] 延遲更新，給後端時間完成檔案寫入
      // 使用漸進式重試：1秒、3秒、5秒，確保顯示最新資料
      console.log('⚡ [課程總覽上傳完成] 排程漸進式更新');
      try {
        // 🔥 第一次更新：1秒後（給後端時間寫入檔案）
        setTimeout(async function() {
          try {
            console.log('🔄 [第1次更新] 從 API 獲取課程總覽資料...');
            await loadUploadedRecordsForCurrentCourse({ force: true });
            console.log('✅ [第1次更新] 課程總覽資料已更新');
          } catch (e) {
            console.warn('⚠️ [第1次更新] 失敗:', e);
          }
        }, 1000);  // 1秒
        
        // 🔥 第二次更新：3秒後（處理慢速網路或大檔案）
        setTimeout(async function() {
          try {
            console.log('🔄 [第2次更新] 檢查是否有新資料...');
            await loadUploadedRecordsForCurrentCourse({ force: true, silent: true });
            console.log('✅ [第2次更新] 完成');
          } catch (e) {
            console.warn('⚠️ [第2次更新] 失敗:', e);
          }
        }, 3000);  // 3秒
        
      } catch (e) {
        console.warn('⚠️ 排程更新失敗:', e);
      }
      
      // 🔥 課程總覽：減少輪詢次數（避免頁面跳動）
      // 課程總覽區域較少更新，只檢查 2 次即可
      try {
        var videoCheckCount = 0;
        var maxVideoChecks = 2;  // 減少到 2 次
        var videoCheckInterval = 5000;  // 增加間隔到 5 秒
        
        var checkOverviewVideoThumbnails = function() {
          videoCheckCount++;
          if (videoCheckCount > maxVideoChecks) {
            console.log('⏱️ [課程總覽-影片縮圖檢查] 已達到最大檢查次數，停止輪詢');
            return;
          }
          
          console.log('🔍 [課程總覽-影片縮圖檢查] 第', videoCheckCount, '次檢查（靜默模式）');
          // 使用靜默模式（force: false），避免頁面跳動
          loadUploadedRecordsForCurrentCourse({ force: false, silent: true });
          
          if (videoCheckCount < maxVideoChecks) {
            setTimeout(checkOverviewVideoThumbnails, videoCheckInterval);
          }
        };
        
        setTimeout(checkOverviewVideoThumbnails, 5000);  // 延遲 5 秒後開始
      } catch (e) {
        console.warn('⚠️ 啟動課程總覽影片縮圖檢查失敗:', e);
      }
      
      try { schedulePostUploadRefresh(1); } catch (e) {}  // 减少补抓次数（2→1）
  ...
  async function uploadOverviewTextOnly(options) {
    ...
    try {
      ...
      // 🔥 使用專門的純文字保存端點（不處理檔案上傳）
      var response = await fetch('/api/learning-records/save', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          isOverview: true,
          overviewSummary: autoBlock,
          coursePeriod: coursePeriod || (metaPath && metaPath.coursePeriod),
          relativePath: metaPath && metaPath.relativePathUnified,
          textOnly: true,
          // 文字欄位
          ov_type: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_type')),
          ov_date: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_date')),
          ov_names: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_names')),
          ov_count: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_count')),
          ov_teacher: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_teacher')),
          ov_topic: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_topic')),
          ov_perf: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_perf')),
          ov_issue: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_issue')),
          ov_solution: (function(el) { return el ? el.value : ''; })(document.getElementById('ov_solution'))
        }),
        credentials: 'include'
      });
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          console.warn('⚠️ [文字上傳] 後端返回失敗:', result.message || result.error);
        }
      } else {
        console.error('❌ [文字上傳] HTTP 錯誤:', response.status);
      }
  ...
  function scheduleProcessingVideosCheck() {
    ...
    console.log('⏰ 排程影片處理狀態檢查（10秒後執行）');
    processingVideosCheckTimer = setTimeout(function() {
  ...
  function renderDrawerFromState() {
    try {
      ...
      var localVideoBlocks = localVideos.map(function (url) {
        ...
        '</div>';
          ...
          var photoBlocks = photos.map(function (pItem) {
            ...
            if (isNewSystem) {
              // 新系統：使用 /api/media/photos/:photoId/preview
            ...
            var removeHandler = "deleteStudentFile('" + (r.studentName || '').replace(/'/g, "\\'") + "', '" + filename.replace(/'/g, "\\'") + "')";
      ...
      var videoBlocks = validVideos.map(function (vItem) {
        var isNewSystem = (typeof vItem === 'object' && vItem.id);
        var filename = isNewSystem ? vItem.filename : vItem;
        var videoId = isNewSystem ? vItem.id : null;
        var url, thumbUrl;
        
        if (isNewSystem) {
          // 新系統：使用 /api/media/videos/:id/download 和 /thumbnail
          url = '/api/media/videos/' + videoId + '/download';
          // 🔥 [修復] 只有當 thumbnailFilename 存在且非空時才設置縮圖 URL
          if (vItem.thumbnailFilename && vItem.thumbnailFilename.trim() !== '') {
            thumbUrl = '/api/media/videos/' + videoId + '/thumbnail';
            console.log('🎬 [課程總覽-抽屜] 新系統影片有縮圖:', { videoId: videoId, thumbnailFilename: vItem.thumbnailFilename });
          } else {
            thumbUrl = '';
            console.log('⏭️ [課程總覽-抽屜] 新系統影片無縮圖，跳過:', { videoId: videoId, thumbnailFilename: vItem.thumbnailFilename });
          }
        } else {
          // 舊系統：使用傳統 buildRecordFileUrl
          url = buildRecordFileUrl(r, filename);
      ...
      // 🔥 智能生成影片縮圖
      try {
        SmartPosterGenerator.processContainer(drawer);
      } catch (e) {
  ...
  function renderUploadedRecords(data) {
    ...
    if (data.overview) {
      ...
      if (Array.isArray(data.overview.newMediaPhotos) && data.overview.newMediaPhotos.length > 0) {
        // 新媒體系統：使用 newMediaPhotos (物件陣列)
        photoItems = data.overview.newMediaPhotos;
        console.log('📸 [課程總覽-新系統] 照片:', photoItems.length, '張');
      } else {
        // 舊系統：使用 files（字串陣列）
        var files = Array.isArray(data.overview.files) ? data.overview.files : [];
        photoItems = files.filter(function(f){ return isImageFilename(f) && !isGeneratedThumbnailName(f); });
        console.log('📸 [課程總覽-舊系統] 照片:', photoItems.length, '張');
      }
      
      if (Array.isArray(data.overview.newMediaVideos) && data.overview.newMediaVideos.length > 0) {
        // 新媒體系統：使用 newMediaVideos (物件陣列)
        videoItems = data.overview.newMediaVideos;
        console.log('🎬 [課程總覽-新系統] 影片:', videoItems.length, '個');
      } else {
        // 舊系統：使用 files（字串陣列）
        var files = Array.isArray(data.overview.files) ? data.overview.files : [];
        videoItems = files.filter(function(f){ return isVideoFilename(f); });
        console.log('🎬 [課程總覽-舊系統] 影片:', videoItems.length, '個');
      }
      
      var rel = data.overview.relativePath || '';
      
      // 🔥 確保使用正確的完整 coursePeriod（例如："SPIKE 三 18:30-20:30 第8週"）
      var coursePeriod = '';
      var courseDate = '';
      ...
      // 🔥 照片渲染：課程總覽統一使用舊 API（檔案直接存取，無需 meta 文件）
      var photoBlocks = photoItems.map(function(pItem){
        var isNewSystem = (typeof pItem === 'object' && pItem.id);
        var filename = isNewSystem ? pItem.filename : pItem;
        
        // 🔥 [重要] 課程總覽統一使用舊 API（避免 404，因為大部分沒有 photos-meta.json）
        var url = buildUrl(filename);
        var displayName = isNewSystem ? (pItem.originalName || filename) : filename;
        var removeHandler = "return deleteOverviewFile('" + String(filename).replace(/'/g, "\\'") + "')";
        
        console.log('📸 [課程總覽-統一舊API] 照片:', filename, '→', url);
      ...
      var videoBlocks = videoItems.map(function(vItem){
        ...
        if (isNewSystem) {
          ...
          // 🔥 檢查新媒體系統是否有縮圖
          if (vItem.thumbnailFilename || vItem.thumbnailRelative) {
            // 新系統有縮圖：使用新媒體縮圖 API
            thumbUrl = '/api/media/videos/' + videoId + '/thumbnail';
            console.log('🎬 [課程總覽-新系統] 使用新媒體縮圖:', { 
              videoId: videoId, 
              thumbUrl: thumbUrl,
              thumbnailFilename: vItem.thumbnailFilename
            });
          } else {
            // 🔥 新系統沒有縮圖：回退到舊系統縮圖映射
            // 🎯 需要匹配檔名：新媒體系統可能是 .mov，但舊系統映射可能是 .webm
            var thumbName = null;
            
            // 方法1：嘗試直接匹配 filename
            if (overviewThumbMap && overviewThumbMap[filename]) {
              thumbName = overviewThumbMap[filename];
            } else {
              // 方法2：如果 filename 是 .mov，嘗試匹配 .webm 版本
              if (filename && filename.indexOf('.mov') > -1) {
                var webmFilename = filename.replace(/\.mov$/i, '.webm');
                if (overviewThumbMap && overviewThumbMap[webmFilename]) {
                  thumbName = overviewThumbMap[webmFilename];
                  console.log('🔄 [回退匹配] .mov → .webm:', filename, '→', webmFilename);
                }
              }
              
              // 方法3：如果還是沒有，嘗試遍歷所有 key 找到匹配的（模糊匹配）
              if (!thumbName && overviewThumbMap) {
                var baseName = filename.replace(/\.(mov|webm|mp4)$/i, '');
                Object.keys(overviewThumbMap).forEach(function(key) {
                  if (key.indexOf(baseName) === 0) {
                    thumbName = overviewThumbMap[key];
                    console.log('🔄 [回退匹配] 模糊匹配:', filename, '→', key);
                  }
                });
              }
            }
            ...
            if (thumbName) {
              thumbUrl = buildUrl(thumbName);
              console.log('🔄 [課程總覽-新系統回退] 新系統無縮圖，使用舊系統:', { 
                videoId: videoId,
                filename: filename,
                thumbName: thumbName,
                thumbUrl: thumbUrl,
                'overviewThumbMap keys': Object.keys(overviewThumbMap)
              });
            } else {
              // 🔥 [修復] 連舊系統也沒有縮圖：不設置 thumbUrl，避免 404
              ...
              console.log('⚠️ [課程總覽-新系統] 新舊系統皆無縮圖，不設置 thumbUrl（避免 404）:', { 
                ...
                filename: filename,
                'overviewThumbMap keys': Object.keys(overviewThumbMap || {}),
                '備註': '將由 SmartPosterGenerator 從本地生成（如果 video src 有效）'
              });
            }
          }
        } else {
          ...
          var thumbName = overviewThumbMap && overviewThumbMap[filename];
          thumbUrl = thumbName ? buildUrl(thumbName) : '';
          console.log('🎬 [課程總覽-舊系統] 影片URL:', { 
            filename: filename, 
            thumbName: thumbName, 
            thumbUrl: thumbUrl,
            'overviewThumbMap 存在': !!overviewThumbMap
          });
        }
        
        // 🔥 [重要] 統一使用 forceReady: true（與學生抽屜一致，確保縮圖持久顯示）
        var displayName = isNewSystem ? (vItem.originalName || filename) : filename;
        var removeHandler = "return deleteOverviewFile('" + String(filename).replace(/'/g, "\\'") + "')";
        
        // 🔍 調試：最終渲染參數
        console.log('🎬 [課程總覽-最終渲染] 影片:', {
          filename: displayName,
          previewUrl: url,
          thumbUrl: thumbUrl,
          lazy: false,
        ...
        return buildMediaPreviewHtml({
          ...
          removeHandler: removeHandler,
          lazy: false,
          forceReady: true  // 🔥 [關鍵] 強制顯示，不依賴緩存計算
        });
      }).join('');
      ...
      try {
        ...
        if (host) {
          ...
          // SmartPosterGenerator.processContainer(host);
    ...
    var studentsHtml = students.length ? students.map(function (r) {
      ...
      var photoBlocks = filteredPhotos.map(function (pItem) {
        ...
        if (isNewSystem) {
          ...
          // 舊系統：pItem 直接就是 filename 字串
          filename = typeof pItem === 'string' ? pItem : String(pItem);
        }
        ...
        var url;
        if (isNewSystem) {
          // 新系統：使用 /api/media/photos/:photoId/preview + 查詢參數
          var coursePeriod = r.coursePeriod || '';
          var date = r.date || '';
          var studentName = r.studentName || '';
      ...
      var videoBlocks = suppressServerVideos ? '' : filteredVideos.map(function (vItem) {
        ...
        if (isNewSystem) {
          ...
          // 舊系統：vItem 直接就是 filename 字串
          filename = typeof vItem === 'string' ? vItem : String(vItem);
        }
        ...
        if (isNewSystem) {
          // 新系統：使用 /api/media/videos/:id/download
          url = '/api/media/videos/' + videoId + '/download';
          console.log('🎬 [抽屜-新系統] 影片下載 URL:', { videoId: videoId, filename: filename, url: url });
        } else {
          // 舊系統：使用傳統 buildRecordFileUrl
          url = buildRecordFileUrl(r, filename);
        }
        ...
        if (isNewSystem && vItem.thumbnailFilename) {
          // 新系統且有縮圖：使用 /api/media/videos/:id/thumbnail
          thumbUrl = '/api/media/videos/' + videoId + '/thumbnail';
          console.log('🎬 [抽屜-新系統] 縮圖 URL:', { 
            videoId: videoId, 
            thumbUrl: thumbUrl, 
            thumbnailFilename: vItem.thumbnailFilename,
            status: vItem.status 
          });
        } else if (isNewSystem && !vItem.thumbnailFilename) {
          // 🔥 [修復] 縮圖未生成，不設置 thumbUrl，避免 404 錯誤
          console.log('⏭️ [抽屜-新系統] 縮圖未生成，跳過 thumbUrl 設定:', { 
            videoId: videoId, 
            status: vItem.status, 
            filename: filename, 
            '備註': '避免 404 錯誤'
          ...
          // 舊系統：從 thumbMapDrawer 查找
          var thumbName = thumbMapDrawer && thumbMapDrawer[filename];
          thumbUrl = thumbName ? buildRecordFileUrl(r, thumbName) : '';
          console.log('🎬 [抽屜-舊系統] 影片:', { 
            filename: filename, 
            thumbName: thumbName, 
            thumbUrl: thumbUrl,
            'thumbMapDrawer 存在': !!thumbMapDrawer
          });
        }
  ...
  function jumpToRecord(recordJson) {
    try {
      ...
      // 提取時間（匹配 HH:MM-HH:MM 格式）
      var timeMatch = courseStr.match(/(\d{1,2}:\d{2})-(\d{1,2}:\d{2})/);
      var startTime = '';
      var endTime = '';
  ...
  function extractTeacherFromRecord(record, debugMode) {
    ...
    if (record.path) {
      var pathParts = record.path.split('/');
      ...
      if (debugMode) console.log('⚠️ [提取講師] 記錄中沒有 path 欄位');
    }
  ...
  async function loadHistory() {
    try {
      ...
      container.innerHTML = groupedArray.map(function (group, index) {
        ...
        // 🔥 課程總覽內容
        var overviewComment = overview ? (overview.comment || '') : '';
  ...
  function clearNewUploadPreviews(studentIndex) {
    ...
    ['photos', 'videos'].forEach(function(type) {
      ...
      Array.prototype.slice.call(container.querySelectorAll('.file-preview.new-upload')).forEach(function (node) {
        ...
        // 🔥 [修復] 保持覆蓋層顯示「同步中」，直到 promoteAwaitingPreviews 移除整個節點
